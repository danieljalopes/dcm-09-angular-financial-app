...
var BrowserAPIs = class {
	...
	*/
	static matchMedia(query) {
		if (Environment.isSSR() || typeof window.matchMedia !== "function") return null;
		try {
...
var fnIds = /* @__PURE__ */ new WeakMap();
var fnSeq = 0;
var Utils$1 = class Utils {
	...
	*/
	static extend(target, source) {
		if (!this.isObject(target) && this.isObject(source)) return this.clone(source);
	...
	*/
	static clone(source, visited = /* @__PURE__ */ new WeakMap(), shallow = false) {
		...
		if (Array.isArray(source)) if (shallow) cloneResult = source.slice();
			...
			for (let i2 = 0; i2 < source.length; i2++) cloneResult[i2] = this.clone(source[i2], visited, false);
		}
	...
	*/
	...
	static hexToRgba(hex = "#999999", opacity = .6) {
		...
		for (let i2 = 0; i2 < h2.length; i2++) h2[i2] = parseInt(h2[i2].length === 1 ? h2[i2] + h2[i2] : h2[i2], 16);
	...
	*/
	static relativeLuminance([r2, g, b]) {
		...
		return .2126 * channel(r2) + .7152 * channel(g) + .0722 * channel(b);
	}
	/**
	...
	ceilToBoundary(timestamp, unit, step, isUTC) {
		...
		if (isUTC) switch (unit) {
			case "second": {
				...
				if (aligned === s2 && d.getUTCMilliseconds() === 0) return timestamp;
			...
			case "hour": {
				...
				if (aligned === h2 && d.getUTCMinutes() === 0 && d.getUTCSeconds() === 0 && d.getUTCMilliseconds() === 0) return timestamp;
				d.setUTCMilliseconds(0);
				d.setUTCSeconds(0);
				d.setUTCMinutes(0);
			...
			case "hour": {
				const h2 = d.getHours();
				const aligned = Math.ceil(h2 / step) * step;
				if (aligned === h2 && d.getMinutes() === 0 && d.getSeconds() === 0 && d.getMilliseconds() === 0) return timestamp;
				d.setMilliseconds(0);
				d.setSeconds(0);
				d.setMinutes(0);
				d.setHours(aligned);
	...
	*/
	...
	setLabelFormatters() {
		...
		if (w.config.xaxis.labels.formatter !== void 0) fmt.xLabelFormatter = w.config.xaxis.labels.formatter;
			...
			if (Utils$1.isNumber(val)) {
				...
				if (w.globals.isBarHorizontal) {
					if (w.globals.maxY - w.globals.minY < 4) return val.toFixed(1);
				}
				return val.toFixed(0);
			}
			return val;
		...
		w.config.yaxis.forEach((yaxe, i2) => {
			if (yaxe.labels.formatter !== void 0) fmt.yLabelFormatters[i2] = yaxe.labels.formatter;
			...
			} else fmt.yLabelFormatters[i2] = (val) => {
				if (!w.globals.xyCharts) return val;
				if (Array.isArray(val)) return val.map((v) => {
	...
	heatmapLabelFormatters() {
		...
		if (w.config.chart.type === "heatmap") {
			...
			const longest = w.seriesData.seriesNames.reduce((a2, b) => a2.length > b.length ? a2 : b, 0);
				...
				radialBar: {
					...
					dataLabels: {
						...
						total: {
							...
							*/
							formatter(w) {
								return w.globals.seriesTotals.reduce((a2, b) => a2 + b, 0) / w.seriesData.series.length + "%";
							}
						}
					},
					barLabels: {
...
var TYPE_OWNED = "_apexOwnedByType";
var ownedBy = (types, fn) => {
	const marked = fn;
	marked[TYPE_OWNED] = types;
	return fn;
};
var TYPE_OWNED_PATHS = [
	"tooltip.custom",
	"tooltip.shared",
	"tooltip.intersect",
...
var writePath = (obj, path, value) => {
	...
	if (value === void 0) delete cur[last];
	else cur[last] = value;
};
var typeOpts = (type, config) => {
	var _a, _b;
	const base = TYPE_ALIASES[type];
	return {
		chart: {
			type: base || type,
...
var isUntouched = (current, fromDefault) => {
...
var Defaults = class Defaults {
	...
	*/
	static forType(opts) {
		...
		else if (requestedType === "histogram") chartDefaults = defaults.histogram();
		else if (requestedType === "waterfall") chartDefaults = defaults.waterfall();
		else if (requestedType === "dumbbell") chartDefaults = defaults.dumbbell();
		else if (requestedType === "streamgraph") chartDefaults = defaults.streamgraph();
		else if (requestedType === "raincloud") chartDefaults = defaults.raincloud();
	...
	*/
	static handOverTypeDefaults(config, fromType, options2) {
		...
		const from = Utils$1.extend(base, Defaults.forType(typeOpts(fromType, config)));
	...
	*/
	...
	waterfall() {
		...
		return __spreadProps(__spreadValues({}, range), {
			chart: {
				stacked: false,
				zoom: { enabled: false },
				animations: { animateGradually: { enabled: true } }
			},
			plotOptions: __spreadProps(__spreadValues({}, range.plotOptions), { bar: __spreadProps(__spreadValues(
				{},
				/** @type {any} */
				range.plotOptions.bar
			), {
				columnWidth: "60%",
				barHeight: "60%"
			}) }),
			dataLabels: __spreadProps(__spreadValues(
				{},
				/** @type {any} */
				range.dataLabels
			), {
		...
		return __spreadProps(__spreadValues({}, this.bar()), {
			...
			tooltip: {
				...
				x: { formatter: (val, opts) => {
					...
					if (!Array.isArray(edges) || typeof k !== "number" || k < 0) return String(val);
					const lo = edges[k];
					const hi = edges[k + 1];
	...
	candlestick() {
		return {
			...
			tooltip: {
				shared: true,
				custom: ownedBy(["candlestick"], ({ seriesIndex, dataPointIndex, w }) => {
	...
	boxPlot() {
		return {
			...
			tooltip: {
				shared: true,
				custom: ownedBy(["boxPlot"], ({ seriesIndex, dataPointIndex, w }) => {
					return this._getBoxTooltip(w, seriesIndex, dataPointIndex, [
						"Minimum",
						"Q1",
						"Median",
	...
	violin() {
		return {
			...
			tooltip: {
				shared: true,
				custom: ownedBy(["violin"], ({ seriesIndex, dataPointIndex, w }) => {
	...
	raincloud() {
		...
		const horizontal = ((_c = (_b = (_a = this.opts) == null ? void 0 : _a.plotOptions) == null ? void 0 : _b.bar) == null ? void 0 : _c.horizontal) === true;
		return __spreadProps(__spreadValues({}, base), {
			...
			tooltip: __spreadProps(__spreadValues({}, base.tooltip), { custom: ownedBy(["raincloud"], ({ seriesIndex, dataPointIndex, w }) => {
	...
	dumbbell() {
		const range = this.rangeBar();
		const endpointRow = (name2, color, value) => "<div class=\"apexcharts-tooltip-dumbbell-endpoint\"><span class=\"series-name\" style=\"color: " + color + "\">" + name2 + "</span> <span class=\"value\">" + value + "</span></div>";
		const handleDumbbellTooltip = (opts) => {
			...
			if (!dumbbell || dumbbell.form !== "series") {
				const { color, seriesName, ylabel: ylabel2, start, end } = getRangeValues(opts);
				return buildRangeTooltipHTML(__spreadProps(__spreadValues({}, opts), {
					color,
					seriesName,
					ylabel: ylabel2,
			...
			const formatter = w.globals.tooltip.tooltipLabels.getFormatters(seriesIndex);
			...
			for (let k = 0; k < values.length; k++) {
				...
				shown.push(values[k]);
				rows += endpointRow(dumbbell.names[k], w.globals.colors[k], formatter.yLbFormatter(values[k]));
			}
			...
			return "<div class=\"apexcharts-tooltip-rangebar apexcharts-tooltip-dumbbell\"><div><span class=\"category\">" + ylabel + "</span></div>" + rows + gap + "</div>";
		};
		return __spreadProps(__spreadValues({}, range), {
			chart: {
				stacked: false,
			...
			plotOptions: __spreadProps(__spreadValues({}, range.plotOptions), { bar: __spreadProps(__spreadValues(
				...
				barHeight: 6,
				columnWidth: 6,
				dumbbell: { dataLabels: { enabled: true } }
			}) }),
			dataLabels: __spreadProps(__spreadValues(
				{},
				/** @type {any} */
	...
	streamgraph() {
		...
		const handleStreamTooltip = (opts) => {
			...
			const active = (() => {
				...
				if (!svg || clientY == null || !span || !isFinite(span)) return seriesIndex;
				const rect = svg.getBoundingClientRect();
				const zoom = w.globals.svgWidth ? rect.width / w.globals.svgWidth : 1;
				const py = (clientY - rect.top) / (zoom || 1) - w.layout.translateY;
				const h2 = w.layout.gridHeight;
				const yPx = (v) => {
					var _a2;
					const frac = (v - w.globals.minY) / span;
					return ((_a2 = w.config.yaxis[0]) == null ? void 0 : _a2.reversed) ? frac * h2 : h2 - frac * h2;
				};
	...
	*/
	...
	stackedBars() {
		...
		return __spreadProps(__spreadValues({}, barDefaults), { plotOptions: __spreadProps(__spreadValues({}, barDefaults.plotOptions), { bar: __spreadProps(__spreadValues({}, barDefaults.plotOptions.bar), { borderRadiusApplication: "end" }) }) });
	}
	/**
	* @param {Record<string, any>} opts
	*/
	...
	*/
	convertCatToNumericXaxis(opts, cats) {
		...
		if (cats && cats.length) labels = cats.map((c2) => {
	...
	*/
	_getBoxTooltip(w, seriesIndex, dataPointIndex, labels, chartType) {
		...
		else return `<div class="apexcharts-tooltip-box apexcharts-tooltip-${w.config.chart.type}"><div>${labels[0]}: <span class="value">` + o2 + `</span></div><div>${labels[1]}: <span class="value">` + h2 + "</span></div>" + (m ? `<div>${labels[2]}: <span class="value">` + m + "</span></div>" : "") + `<div>${labels[3]}: <span class="value">` + l2 + `</span></div><div>${labels[4]}: <span class="value">` + c2 + "</span></div></div>";
	}
	/**
	...
	*/
	_getRaincloudTooltip(w, seriesIndex, dataPointIndex) {
		var _a, _b;
		const summary = (_a = w.violinData.seriesViolinSummary[seriesIndex]) == null ? void 0 : _a[dataPointIndex];
		if (!summary) return this._getViolinTooltip(w, seriesIndex, dataPointIndex);
		const pts = ((_b = w.violinData.seriesViolinPoints[seriesIndex]) == null ? void 0 : _b[dataPointIndex]) || [];
		const name2 = w.config.series[seriesIndex].name || "series-" + (seriesIndex + 1);
		const fmt = (v) => {
...
var Config = class {
	...
	*/
	...
	init({ responsiveOverride }) {
		...
		if (opts && typeof opts === "object") {
			...
			if ((_a = opts.chart.brush) == null ? void 0 : _a.enabled) chartDefaults = defaults.brush(chartDefaults);
			if ((_c = (_b = opts.plotOptions) == null ? void 0 : _b.line) == null ? void 0 : _c.isSlopeChart) chartDefaults = defaults.slope();
			if (opts.chart.stacked && opts.chart.stackType === "100%") opts = defaults.stacked100(opts);
			if (((_e = (_d = opts.plotOptions) == null ? void 0 : _d.bar) == null ? void 0 : _e.isDumbbell) && opts.chart.requestedType !== "dumbbell") opts = defaults.dumbbellSizing(opts);
			...
			if (((_f = opts.chart.sparkline) == null ? void 0 : _f.enabled) || ((_h = (_g = Environment.getApex().chart) == null ? void 0 : _g.sparkline) == null ? void 0 : _h.enabled)) chartDefaults = defaults.sparkline(chartDefaults);
	...
	*/
	normalizeAliasedChartType(opts) {
		...
		if (!requested || !TYPE_ALIASES[requested]) return opts;
		opts.chart.requestedType = requested;
		if (requested === "waffle") {
			opts.plotOptions = opts.plotOptions || {};
			opts.plotOptions.unit = opts.plotOptions.unit || {};
			if (opts.plotOptions.unit.layout == null) opts.plotOptions.unit.layout = "grid";
			if (opts.plotOptions.unit.shape == null) opts.plotOptions.unit.shape = "square";
			...
			if (opts.plotOptions.bar.horizontal == null) opts.plotOptions.bar.horizontal = true;
			opts.chart.stacked = false;
			opts.chart.type = "rangeBar";
		} else if (requested === "streamgraph") {
			opts.chart.stacked = false;
			opts.yaxis = opts.yaxis || {};
			if (!Array.isArray(opts.yaxis) && opts.yaxis.show == null) opts.yaxis.show = false;
			opts.chart.type = "rangeArea";
		} else if (requested === "histogram") {
			opts.xaxis = opts.xaxis || {};
			if (opts.xaxis.type == null) opts.xaxis.type = "numeric";
			opts.chart.type = "bar";
		} else if (requested === "raincloud") opts.chart.type = "violin";
		return opts;
	}
	/**
	* @param {string} chartType
	...
	*/
	extendYAxis(opts, w) {
		...
		if (isLogY && series.length !== opts.yaxis.length && series.length) opts.yaxis = series.map((s2, i2) => {
			if (!s2.name) series[i2].name = `series-${i2 + 1}`;
			if (opts.yaxis[i2]) {
	...
	*/
	defineLazyInitialSeries(globals) {
		...
		Object.defineProperty(globals, "initialSeries", {
			...
			enumerable: true,
			get() {
				if (snap === null) snap = Utils$1.clone(src);
				return snap;
			},
			set(value) {
				src = Array.isArray(value) ? value.map((s2) => s2 && typeof s2 === "object" && !Array.isArray(s2) ? __spreadValues({}, s2) : s2) : value;
				snap = null;
...
var CoreUtils = class {
	...
	*/
	static checkComboSeries(series, chartType) {
		...
		if (series.length && typeof series[0].type !== "undefined") series.forEach((s2) => {
			if (s2.type === "bar" || s2.type === "column" || s2.type === "candlestick" || s2.type === "boxPlot" || s2.type === "violin") comboBarCount++;
			if (typeof s2.type !== "undefined" && s2.type !== chartType) comboCount++;
		});
	...
	**/
	getStackedSeriesTotals(excludedSeriesIndices = []) {
		...
		for (let i2 = 0; i2 < w.seriesData.series[w.globals.maxValsInArrayIndex].length; i2++) {
			let t2 = 0;
			for (let j = 0; j < w.seriesData.series.length; j++) if (typeof w.seriesData.series[j][i2] !== "undefined" && excludedSeriesIndices.indexOf(j) === -1) t2 += w.seriesData.series[j][i2];
			total.push(t2);
		}
		return total;
	}
				...
				seriesNames.forEach((name2) => {
					cnf.series.forEach((s2, si) => {
						if (s2.name === name2) {
							...
							else console.warn("Series '" + s2.name + "' referenced more than once in what looks like the new style. That is, when using either seriesName: [], or when there are more series than yaxes.");
							else {
	...
	*/
	isSeriesNull(index = null) {
		...
		if (index === null) r2 = series.filter((d) => d !== null);
		else if (series[index] && Array.isArray(series[index].data)) r2 = series[index].data.filter((d) => d !== null);
		else r2 = series[index] !== null && series[index] !== void 0 ? [series[index]] : [];
		return r2.length === 0;
	}
	/**
	* @param {number} index
	*/
	seriesHaveSameValues(index) {
		...
		return seriesAtIndex.every((val, i2, arr) => val === arr[0]);
	}
	/**
	* @param {any[]} labels
	*/
	getCategoryLabels(labels) {
		...
		if (w.config.xaxis.convertedCatToNumeric) catLabels = labels.map((i2) => {
	...
	getLargestSeries() {
		const w = this.w;
		w.globals.maxValsInArrayIndex = w.seriesData.series.map((a2) => a2.length).indexOf(Math.max.apply(
			Math,
			/**
			* @param {number[]} a
			*/
	...
	**/
	...
	getCalculatedRatios() {
		...
		let baseLineX = 0;
		gl.yRange = [];
		if (gl.isMultipleYAxis) for (let i2 = 0; i2 < gl.minYArr.length; i2++) {
		...
		for (let i2 = 0; i2 < gl.yRange.length; i2++) yRatio.push(gl.yRange[i2] / this.w.layout.gridHeight);
		...
		if (w.globals.seriesYAxisReverseMap.length > 0) {
			...
			if (gl.isMultipleYAxis) {
				baseLineY = [];
				for (let i2 = 0; i2 < yRatio.length; i2++) baseLineY.push(scaleBaseLineYScale(gl.minYArr[i2], i2));
			} else {
				baseLineY = [];
	...
	*/
	getLogSeries(series) {
		...
		w.globals.seriesLog = series.map((s2, i2) => {
			...
			if (w.config.yaxis[yAxisIndex] && w.config.yaxis[yAxisIndex].logarithmic) return s2.map((d) => {
	...
	*/
	getLogYRatios(yRatio) {
		...
		_gl.logYRange = gl.yRange.map((_, i2) => {
			const yAxisIndex = w.globals.seriesYAxisReverseMap[i2];
...
function pathBbox(arr) {
	...
	arr.forEach((cmd) => {
		for (let i2 = 1; i2 < cmd.length; i2 += 2) if (i2 + 1 <= cmd.length) {
			const x = cmd[i2];
			const y = cmd[i2 + 1];
...
function findNextM(arr, offset) {
	...
	for (var i2 = offset, len = arr.length; i2 < len; ++i2) if (arr[i2][0] == "M") return i2;
	return false;
}
function arcToBezier(pos, val) {
	...
	if (largeArcFlag === sweepFlag) t2 *= -1;
...
function handleBlock(startArr, startOffsetM, startOffsetNextM, destArr, destOffsetM, destOffsetNextM) {
	...
	while (true) {
		...
		if (i2 == startArrTemp.length) startArrTemp.push([
			"C",
			posStart.pos[0],
			posStart.pos[1],
			posStart.pos[0],
		...
		if (i2 == destArrTemp.length) destArrTemp.push([
			"C",
			posDest.pos[0],
			posDest.pos[1],
			posDest.pos[0],
...
function morphPolygons(fromD, toD, n2 = 96) {
	...
	for (let i2 = 0; i2 < n2; i2++) aligned[i2] = fromPts[(i2 + bestOffset) % n2];
	return function(pos) {
		let out = "";
		for (let i2 = 0; i2 < n2; i2++) {
			const a2 = aligned[i2];
...
function easeInOut(t2) {
	return -Math.cos(t2 * Math.PI) / 2 + .5;
}
var _defaultEasing = easeInOut;
		...
		const run = () => {
			...
			const tick = (now) => {
				...
				if (rawPos < 1) BrowserAPIs.requestAnimationFrame(tick);
					...
					if (this._plotTarget) el.attr("d", this._plotSnap || this._plotTarget);
					if (this._afterCb) this._afterCb.call(el);
				}
			};
...
REGISTRY.set("easeInOutQuad", (t2) => t2 < .5 ? 2 * t2 * t2 : 1 - Math.pow(-2 * t2 + 2, 2) / 2);
REGISTRY.set("easeInCubic", (t2) => t2 * t2 * t2);
REGISTRY.set("easeOutCubic", (t2) => 1 - Math.pow(1 - t2, 3));
REGISTRY.set("easeInOutCubic", (t2) => t2 < .5 ? 4 * t2 * t2 * t2 : 1 - Math.pow(-2 * t2 + 2, 3) / 2);
REGISTRY.set("easeOutBack", (t2) => {
	return 1 + 2.70158 * Math.pow(t2 - 1, 3) + 1.70158 * Math.pow(t2 - 1, 2);
});
REGISTRY.set("easeInOutBack", (t2) => {
	const c2 = 1.70158 * 1.525;
	return t2 < .5 ? Math.pow(2 * t2, 2) * (3.5949095 * 2 * t2 - c2) / 2 : (Math.pow(2 * t2 - 2, 2) * (3.5949095 * (t2 * 2 - 2) + c2) + 2) / 2;
});
function registerEasing(name2, fn) {
	if (typeof name2 === "string" && name2 && typeof fn === "function") REGISTRY.set(name2, fn);
}
...
var Animations = class {
	...
	*/
	animatePathsGradually(params) {
		const { el, realIndex, j, fill, pathFrom, pathTo, pathToInterp, speed, delay, scrollMorph } = params;
		const me = this;
		const w = this.w;
		let delayFactor = 0;
		...
		me.morphSVG(el, realIndex, j, w.config.chart.type === "line" && !w.globals.comboCharts ? "stroke" : fill, pathFrom, pathTo, speed, delay * delayFactor, scrollMorph, pathToInterp);
	}
	/**
	...
	*/
	morphSVG(el, realIndex, j, fill, pathFrom, pathTo, speed, delay, scrollMorph, pathToInterp) {
		...
		if (pathToInterp && (!pathToInterp.trim() || pathToInterp.indexOf("undefined") > -1 || pathToInterp.indexOf("NaN") > -1)) pathToInterp = void 0;
		...
		if (w.globals.dataChanged) {
			...
			if (dynEasing != null) morphEase = resolveEasing(dynEasing);
			else if (scrollMorph) morphEase = resolveEasing("linear");
		}
		const runner = el.plot(pathFrom).animate(speed, delay);
		if (morphEase) runner.ease(morphEase);
		runner.plot(pathToInterp || pathTo, morphAlgo, pathToInterp ? pathTo : void 0).after(() => {
			if (Utils$1.isNumber(j)) {
				const maxSeries = w.seriesData.series[w.globals.maxValsInArrayIndex];
				if (maxSeries && j === maxSeries.length - 2 && w.globals.shouldAnimate) this.animationCompleted(el);
...
var Filters = class Filters {
	...
	*/
	getDefaultFilter(el, i2) {
		...
		if (w.config.chart.dropShadow.enabled) this.dropShadow(el, w.config.chart.dropShadow, i2);
	}
	/**
	* @param {any} el
	* @param {number} i
	...
	*/
	applyFilter(el, i2, filterType, intensity) {
		...
		const t2 = Math.max(0, Math.min(1, typeof intensity === "number" ? intensity : fallback));
		const diag = 1 - t2;
		...
		if (el.filterWith) {
          ...
          `,
				...
				if (shadowAttr.enabled) this.addShadow(add, i2, shadowAttr, "brightness");
			});
	...
	*/
	addShadow(add, i2, attrs, source) {
		...
		if (((_a = w.config.chart.dropShadow.enabledOnSeries) == null ? void 0 : _a.length) > 0) {
			if (w.config.chart.dropShadow.enabledOnSeries.indexOf(i2) === -1) return add;
		}
		add.offset({
			in: source,
			dx: left,
	...
	*/
	dropShadow(el, attrs, i2 = 0) {
		...
		if (((_a = w.config.chart.dropShadow.enabledOnSeries) == null ? void 0 : _a.length) > 0) {
			if (((_b = w.config.chart.dropShadow.enabledOnSeries) == null ? void 0 : _b.indexOf(i2)) === -1) return el;
		}
		if (el.filterWith) {
			el.filterWith((add) => {
	...
	setSelectionFilter(el, realIndex, dataPointIndex) {
		...
		if (typeof w.interact.selectedDataPoints[realIndex] !== "undefined") {
			if (w.interact.selectedDataPoints[realIndex].indexOf(dataPointIndex) > -1) {
				...
				if (activeFilter.type !== "none") this.applyFilter(el, realIndex, activeFilter.type, activeFilter.value);
			}
		}
	}
	/**
...
var Graphics = class Graphics {
	...
	**/
	renderPaths({ j, realIndex, pathFrom, pathTo, pathToInterp, stroke, strokeWidth, strokeLinecap, fill, animationDelay, initialSpeed, dataChangeSpeed, className, chartType, shouldClipToGrid = true, bindEventsOnPaths = true, drawShadow = true, drawMask = null, scrollMorph = false }) {
		var _a, _b, _c, _d;
		const w = this.w;
		...
		const fadeOnDataChange = !!((chartType === "candlestick" || chartType === "boxPlot") && shouldAnimate && !useDrawMode && w.globals.dataChanged && !crossTypeMorph);
		...
		if (shouldAnimate && !revealViaFade && (w.globals.dataChanged && dynamicAnim || crossTypeMorph)) anim.animatePathsGradually(__spreadProps(__spreadValues({}, defaultAnimateOpts), { speed: dataChangeSpeed }));
	...
	*/
	drawGradient(style, gfrom, gto, opacityFrom, opacityTo, size = null, stops = null, colorStops = [], i2 = 0, verticalUserSpace = false) {
		...
		if (!colorStops || colorStops.length === 0) g = w.dom.Paper.gradient(radial ? "radial" : "linear", (add) => {
			...
			(Array.isArray(colorStops[i2]) ? colorStops[i2] : Array.isArray(colorStops[0]) ? colorStops[0] || [] : colorStops).forEach((s2) => {
				add.stop(s2.offset / 100, s2.color, s2.opacity);
			});
		});
		if (!radial) {
			if (style === "vertical") if (verticalUserSpace) {
				g.attr({ gradientUnits: "userSpaceOnUse" });
				g.from(0, 0).to(0, w.layout.gridHeight);
			} else g.from(0, 0).to(0, 1);
	...
	drawText({ x, y, text, textAnchor, fontSize, fontFamily, fontWeight, foreColor, opacity, maxWidth, cssClass = "", isPlainText = true, dominantBaseline = "auto" }) {
		...
		if (Array.isArray(text)) elText = w.dom.Paper.text((add) => {
			for (let i2 = 0; i2 < text.length; i2++) {
				...
				if (maxWidth) truncatedText = this.getTextBasedOnMaxWidth(__spreadValues({ text: text[i2] }, commonProps));
...
w.dom.Paper.text((add) => add.tspan(truncatedText));
	...
	*/
	pathMouseEnter(path, e2) {
		...
		if (typeof w.config.chart.events.dataPointMouseEnter === "function") w.config.chart.events.dataPointMouseEnter(e2, this.ctx, {
			seriesIndex: i2,
			dataPointIndex: j,
			w
		});
	...
	*/
	pathMouseLeave(path, e2) {
		...
		const j = parseInt((_b = path.node.getAttribute("j")) != null ? _b : "", 10);
		if (isNaN(i2) || isNaN(j)) return;
		if (typeof w.config.chart.events.dataPointMouseLeave === "function") w.config.chart.events.dataPointMouseLeave(e2, this.ctx, {
			seriesIndex: i2,
			dataPointIndex: j,
			w
		});
		...
		if (Filters.hoverOutlineOwnsHoverState(w)) return;
		if (w.config.states.hover.filter.type !== "none") filters.getDefaultFilter(path, i2);
	}
	/**
	...
	*/
	_togglePointSelection(path, filters, i2, j) {
		...
		if (path.node.getAttribute("selected") === "true") {
			...
			if (index > -1) w.interact.selectedDataPoints[i2].splice(index, 1);
			return "false";
		}
		if (!w.config.states.active.allowMultipleDataPointsSelection && w.interact.selectedDataPoints.length > 0) this._clearAllDataPointSelections(filters, i2);
		path.node.setAttribute("selected", "true");
		if (typeof w.interact.selectedDataPoints[i2] === "undefined") w.interact.selectedDataPoints[i2] = [];
	...
	*/
	_applyPointSelectionFilter(path, filters, i2, selected) {
		...
		if (selected === "true") {
			const activeFilter = w.config.states.active.filter;
			if (activeFilter !== "none") filters.applyFilter(path, i2, activeFilter.type, activeFilter.value);
			...
			const hoverFilter = w.config.states.hover.filter;
			filters.applyFilter(path, i2, hoverFilter.type, hoverFilter.value);
		} else filters.getDefaultFilter(path, i2);
	}
	/**
	* @param {any} path
	* @param {Event | null} e
	*/
	pathMouseDown(path, e2) {
		...
		if (!!!(link && (typeof link.dimension === "function" || link.enabled))) {
			const selected = this._togglePointSelection(path, filters, i2, j);
			this._applyPointSelectionFilter(path, filters, i2, selected);
		}
		if (typeof w.config.chart.events.dataPointSelection === "function") w.config.chart.events.dataPointSelection(e2, this.ctx, {
			selectedDataPoints: w.interact.selectedDataPoints,
			seriesIndex: i2,
			dataPointIndex: j,
			w
		});
		if (e2) Graphics._fireEvent(w, "dataPointSelection", [
			e2,
			this.ctx,
			{
				selectedDataPoints: w.interact.selectedDataPoints,
	...
	*/
	setupEventDelegation(parentGroup, targetSelector) {
		...
		parentGroup.node.addEventListener("mouseover", (e2) => {
			const targetNode = Graphics._findDelegateTarget(e2.target, parentGroup.node, targetSelector);
			...
			if (targetNode.instance) this.pathMouseEnter(targetNode.instance, e2);
		});
		parentGroup.node.addEventListener("mouseout", (e2) => {
			if (!currentHovered) return;
			if ((e2.relatedTarget ? Graphics._findDelegateTarget(e2.relatedTarget, parentGroup.node, targetSelector) : null) !== currentHovered) {
				if (currentHovered && currentHovered.instance) this.pathMouseLeave(
					/** @type {any} */
					currentHovered.instance,
					e2
		...
		parentGroup.node.addEventListener("mousedown", (e2) => {
			const targetNode = Graphics._findDelegateTarget(e2.target, parentGroup.node, targetSelector);
			if (targetNode && targetNode.instance) this.pathMouseDown(targetNode.instance, e2);
		});
	}
	/**
	* @param {import('../types/internal').ChartStateW} w
	...
	*/
	static _fireEvent(w, name2, args) {
		...
		for (let i2 = 0; i2 < handlers.length; i2++) handlers[i2].apply(null, args);
	}
	/**
	* @param {any} node
...
global.SVG;
	...
	*/
	computeColorStops(axisRange, multiColorConfig) {
		...
		if (axisRange.reversed) offset = 100 - offset;
	...
	*/
	fillPath(opts) {
		...
		if (useGradient) {
			...
			let type = (_g = (_f = (_e = opts.fillConfig) == null ? void 0 : _e.gradient) == null ? void 0 : _f.type) != null ? _g : cnf.fill.gradient.type;
			if (drawMultiColorLine) {
				colorStops[this.seriesIndex] = this.computeColorStops(this.getSeriesAxisRange(this.seriesIndex), cnf.plotOptions.line.colors);
				type = "vertical";
			}
	...
	*/
	...
	handleGradientFill({ type, fillColor, fillOpacity, fillConfig, colorStops, i: i2, verticalUserSpace = false }) {
		let fillCnf = this.w.config.fill;
		...
		return graphics.drawGradient(type, gradientFrom, gradientTo, opacityFrom, opacityTo, opts.size, fillCnf.gradient.stops, colorStops, i2, verticalUserSpace);
	}
};
var OK_FILTER_TYPES = [
	"none",
...
function computeMarkCount(w) {
	...
	if (maxLen >= 5e4) total = Math.max(total, maxLen);
	return total;
}
function hasCanvasUnsupportedFeature(w) {
	...
	if (Array.isArray(fillType) ? fillType.some(isUnsupportedFill) : isUnsupportedFill(fillType)) return true;
	const lineColors = (_b = (_a = w.config.plotOptions) == null ? void 0 : _a.line) == null ? void 0 : _b.colors;
	if (lineColors && lineColors.colorAboveThreshold && lineColors.colorBelowThreshold) return true;
...
var Markers = class Markers {
	...
	*/
	_shouldBatch() {
		...
		for (let i2 = 0; i2 < series.length; i2++) {
			...
			const data = (_e = w.config.series[i2]) == null ? void 0 : _e.data;
			...
			if (!(w.globals.markers.size[i2] > 0 || hasNull && m.showNullDataPoints)) continue;
	...
	setGlobalMarkerSize() {
		...
		if (w.globals.markers.size.length > 0) {
			if (w.globals.markers.size.length < w.seriesData.series.length + 1) {
				for (let i2 = 0; i2 <= w.seriesData.series.length; i2++) if (typeof w.globals.markers.size[i2] === "undefined") w.globals.markers.size.push(w.globals.markers.size[0]);
			}
	...
	plotChartMarkers({ pointsPos, seriesIndex, j, pSize, alwaysDrawMarker = false, isVirtualPoint = false }) {
		...
		if (Array.isArray(p.x)) for (let q = 0; q < p.x.length; q++) {
			...
			if (w.globals.markers.largestSize === 0 && w.globals.hasNullValues && w.seriesData.series[i2][j + 1] !== null && !isVirtualPoint) invalidMarker = true;
			if (j === 1 && q === 0) dataPointIndex = 0;
			if (j === 1 && q === 1) dataPointIndex = 1;
			let markerClasses = "apexcharts-marker";
			if (Markers.markersAreInert(w)) markerClasses += " no-pointer-events";
			const shouldMarkerDraw = Array.isArray(w.config.markers.size) ? w.globals.markers.size[seriesIndex] > 0 : w.config.markers.size > 0;
			if (w.globals.markers.batched && (shouldMarkerDraw || alwaysDrawMarker) && !hasDiscreteMarkers && !isVirtualPoint) {
			...
			if (shouldMarkerDraw || alwaysDrawMarker || hasDiscreteMarkers) {
				...
				if (!invalidMarker) {
					if ((w.globals.markers.size[seriesIndex] > 0 || alwaysDrawMarker || hasDiscreteMarkers) && !elMarkersWrap) {
						...
						if (standardWrap && this._seriesWrap && this._seriesWrapIndex === seriesIndex) elMarkersWrap = this._seriesWrap;
						else {
							elMarkersWrap = emit.group({ class: standardWrap ? "apexcharts-series-markers" : "" });
	...
	*/
	_batchPoint(seriesIndex, dataPointIndex, x, y, { invalid, graphics, pSize, trackPoint }) {
		...
		if (invalid) return;
		if (!this._batch || this._batch.seriesIndex !== seriesIndex) this._batch = {
			seriesIndex,
			opts: this.getMarkerConfig({
				cssClass: "",
				seriesIndex
			}),
		...
		if (x < -slack || x > w.layout.gridWidth + slack || y < -slack || y > w.layout.gridHeight + slack) return;
	...
	getMarkerConfig({ cssClass, seriesIndex, dataPointIndex = null, radius = null, size = null, strokeWidth = null }) {
		...
		if (dataPointIndex !== null && m.discrete.length) m.discrete.map((marker) => {
			if (marker.seriesIndex === seriesIndex && marker.dataPointIndex === dataPointIndex) {
				if (marker.strokeColor !== void 0) pStyle.pointStrokeColor = marker.strokeColor;
				if (marker.fillColor !== void 0) pStyle.pointFillColor = marker.fillColor;
	...
	setupMarkerDelegation(parentGroup) {
		...
		parentGroup.node.addEventListener("click", (e2) => {
			if (w.config.markers.onClick) {
				if (Graphics._findDelegateTarget(e2.target, parentGroup.node, selector)) w.config.markers.onClick(e2);
			}
		});
		parentGroup.node.addEventListener("dblclick", (e2) => {
			if (w.config.markers.onDblClick) {
				if (Graphics._findDelegateTarget(e2.target, parentGroup.node, selector)) w.config.markers.onDblClick(e2);
			}
		});
		parentGroup.node.addEventListener("touchstart", (e2) => {
			const targetNode = Graphics._findDelegateTarget(e2.target, parentGroup.node, selector);
			if (targetNode && targetNode.instance) this._graphics.pathMouseDown(targetNode.instance, e2);
		}, { passive: true });
	}
	/**
	* @returns {any}
...
var Scatter = class {
	...
	*/
	draw(elSeries, j, opts) {
		...
		if (!elPointsWrap || this._elPointsWrapParent !== elPointsMain) {
			elPointsWrap = emit.group({ class: `apexcharts-series-markers apexcharts-series-${w.config.chart.type}` });
		...
		if (Array.isArray(pointsPos.x)) for (let q = 0; q < pointsPos.x.length; q++) {
			...
			if (shouldDraw) {
				...
				if (emit.kind === "canvas") {
					if (typeof w.globals.pointsArray[realIndex] === "undefined") w.globals.pointsArray[realIndex] = [];
	...
	*/
	_buildPerSeriesCache(realIndex, emit) {
		...
		return {
			realIndex,
			emit,
			isBubble: w.config.chart.type === "bubble" || w.globals.comboCharts && w.config.series[realIndex] && w.config.series[realIndex].type === "bubble",
	...
	*/
	drawPoint(x, y, radius, realIndex, dataPointIndex, j) {
		...
		if (!ps || ps.realIndex !== realIndex) ps = this._perSeries = this._buildPerSeriesCache(realIndex, seriesEmitter(this.ctx, this.graphics));
		const emit = ps.emit;
		let markerConfig;
		if (ps.canCacheConfig) {
			if (!ps.markerConfig) ps.markerConfig = markers.getMarkerConfig({
		...
		} else markerConfig = markers.getMarkerConfig({
			cssClass: "apexcharts-marker",
			seriesIndex: i2,
			dataPointIndex,
			radius: ps.isBubble ? radius : null
		});
		const dataItem = w.config.series[i2].data[dataPointIndex];
		let pathFillCircle;
		if (ps.fillCircle !== void 0) pathFillCircle = ps.fillCircle;
		else {
			...
			if (ps.fillCacheable === void 0) ps.fillCacheable = ps.canCacheConfig && fill.getFillType(realIndex) === "solid" && typeof markerConfig.pointFillColor === "string" && !!markerConfig.pointFillColor;
			if (ps.fillCacheable && !(dataItem == null ? void 0 : dataItem.fillColor)) ps.fillCircle = pathFillCircle;
		}
		const el = emit.drawMarker(x, y, markerConfig);
		if (dataItem == null ? void 0 : dataItem.fillColor) pathFillCircle = dataItem.fillColor;
		const jt = ps.jitter;
		if ((jt == null ? void 0 : jt.enabled) && jt.distributed && w.globals.colors.length) {
			const bandIdx = Math.round((_a = w.seriesData.seriesX[realIndex]) == null ? void 0 : _a[dataPointIndex]);
		...
		if (ps.doInitialAnim) {
			if (!ps.anim) {
				...
				ps.anim = {
					...
					baseDelay: gradEnabled ? Math.min(20, animCfg.speed * .5 / Math.max(1, totalPoints)) : 0
				};
			}
...
var resolveDataLabelOffset = (value, w, seriesIndex, dataPointIndex) => {
	if (typeof value !== "function") return value;
...
var DataLabels = class {
	...
	*/
	dataLabelsCorrection(x, y, val, i2, dataPointIndex, alwaysDrawDataLabel, fontSize) {
		...
		if (typeof w.globals.dataLabelsRects[i2] === "undefined") w.globals.dataLabelsRects[i2] = [];
	...
	drawDataLabel({ type, pos, i: i2, j, isRangeStart, strokeWidth = 2 }) {
		...
		if (w.globals.collapsedSeriesIndices.indexOf(i2) !== -1 || !dataLabelsConfig.enabled || !Array.isArray(pos.x)) return elDataLabelsWrap;
		...
		for (let q = 0; q < pos.x.length; q++) {
			...
			if (!isNaN(x)) {
				...
				if (type === "rangeArea") if (isRangeStart) val = w.rangeData.seriesRangeStart[i2][dataPointIndex];
				else val = w.rangeData.seriesRangeEnd[i2][dataPointIndex];
				let text = "";
				const getText = (v) => {
				...
				else if (dataPointIndex === w.config.series[i2].data.length - 1) textAnchor = "start";
	...
	*/
	plotDataLabelsText(opts) {
		const w = this.w;
		const graphics = new Graphics(this.w);
		let { x, y, i: i2, j, text, textAnchor, fontSize, parent, dataLabelsConfig, color, alwaysDrawDataLabel, offsetCorrection, className, seriesIndex = i2, dataPointIndex = j } = opts;
		let dataLabelText = null;
		if (Array.isArray(w.config.dataLabels.enabledOnSeries)) {
			if (w.config.dataLabels.enabledOnSeries.indexOf(i2) < 0) return dataLabelText;
		}
		let correctedLabels = {
			x,
			y,
		...
		if (offsetCorrection) correctedLabels = this.dataLabelsCorrection(x, y, text, i2, j, alwaysDrawDataLabel, parseInt(
			/** @type {any} */
		...
		const resolvedOffX = offsetsHandledElsewhere ? 0 : resolveDataLabelOffset(dataLabelsConfig.offsetX, w, seriesIndex, dataPointIndex);
		const resolvedOffY = offsetsHandledElsewhere ? 0 : resolveDataLabelOffset(dataLabelsConfig.offsetY, w, seriesIndex, dataPointIndex);
		let offX = resolvedOffX;
		const offY = resolvedOffY;
		if (w.globals.isSlopeChart) {
			if (j !== 0) offX = resolvedOffX * -2 + 5;
			if (j !== 0 && j !== w.config.series[i2].data.length - 1) offX = 0;
		}
		if (correctedLabels.drawnextLabel) {
			...
			if (typeof w.globals.lastDrawnDataLabelsIndexes[i2] === "undefined") w.globals.lastDrawnDataLabelsIndexes[i2] = [];
	...
	*/
	...
	bringForward() {
		...
		for (let i2 = 0; i2 < elDataLabelsNodes.length; i2++) if (elSeries) elSeries.insertBefore(elDataLabelsNodes[i2], elSeries.nextSibling);
	}
};
...
var AxesUtils = class {
	...
	*/
	getLabel(labels, timescaleLabels, x, i2, drawnLabels = [], fontSize = "12px", isLeafGroup = true) {
		...
		if (isLeafGroup) {
			...
			if (customFormatter !== void 0) label = customFormatter(rawLabel, labels[i2], {
				i: i2,
				dateFormatter: new DateTime(this.w).formatDate,
				w
			});
	...
	*/
	checkLabelBasedOnTickamount(i2, label, labelsLen) {
		...
		if (i2 % Math.round(labelsLen / (ticks + 1)) === 0) return label;
		else label.text = "";
		return label;
	}
	/**
	...
	*/
	checkForOverflowingLabels(i2, label, labelsLen, drawnLabels, drawnLabelsRects) {
		...
		if (w.config.xaxis.labels.hideOverlappingLabels && drawnLabels.length > 0) {
			...
			if (label.x < prev.textRect.width / (w.layout.rotateXLabels ? Math.max(Math.abs(w.config.xaxis.labels.rotate), 1) / 12 : 1.01) + prev.x) label.text = "";
		}
		return label;
	}
	/**
	...
	*/
	checkForReversedLabels(i2, labels) {
		const w = this.w;
		if (w.config.yaxis[i2] && w.config.yaxis[i2].reversed) labels.reverse();
		return labels;
	}
	/**
	* @param {number} index
	...
	*/
	translateYAxisIndex(index) {
		...
		if (w.seriesData.series.length > yaxis.length || yaxis.some((a2) => Array.isArray(a2.seriesName))) return index;
...
var XAxis = class {
	...
	*/
	...
	drawXaxis() {
		...
		let labels = [];
		for (let i2 = 0; i2 < this.xaxisLabels.length; i2++) labels.push(this.xaxisLabels[i2]);
		this.drawXAxisLabelAndGroup(true, graphics, elXaxisTexts, labels, w.axisFlags.isXNumeric, (i2, colWidth) => colWidth);
		if (w.labelData.hasXaxisGroups) {
			const labelsGroup = w.labelData.groups;
			labels = [];
			for (let i2 = 0; i2 < labelsGroup.length; i2++) labels.push(labelsGroup[i2].title);
			...
			this.drawXAxisLabelAndGroup(false, graphics, elXaxisTexts, labels, false, (i2, colWidth) => labelsGroup[i2].cols * colWidth, overwriteStyles);
		}
	...
	*/
	drawXAxisLabelAndGroup(isLeafGroup, graphics, elXaxisTexts, labels, isXNumeric, colWidthCb, overwriteStyles = {}) {
		...
		for (let i2 = 0; i2 <= labelsLen - 1; i2++) {
			...
			if (i2 === 0 && labelsLen === 1 && colWidth / 2 === xPos && dataPoints === 1) x = w.layout.gridWidth / 2;
			let label = this.axesUtils.getLabel(labels, w.labelData.timescaleLabels, x, i2, drawnLabels, xaxisFontSize, isLeafGroup);
			...
			if (typeof w.config.xaxis.tickAmount !== "undefined" && w.config.xaxis.tickAmount !== "dataPoints" && w.config.xaxis.type !== "datetime") label = this.axesUtils.checkLabelBasedOnTickamount(i2, label, labelsLen);
			else label = this.axesUtils.checkForOverflowingLabels(i2, label, labelsLen, drawnLabels, drawnLabelsRects);
			...
			if (w.config.xaxis.labels.show && !fullyOutsideGrid) {
				...
				elText.on("click", (e2) => {
					if (typeof w.config.chart.events.xAxisLabelClick === "function") {
						const opts = Object.assign({}, w, { labelIndex: i2 });
			...
			if (i2 < labelsLen - 1) xPos = xPos + colWidthCb(i2 + 1, colWidth);
		}
	}
	/**
	* @param {number} realIndex
	*/
	drawXaxisInversed(realIndex) {
		...
		const labels = [];
		if (w.config.yaxis[realIndex].show) for (let i2 = 0; i2 < this.xaxisLabels.length; i2++) labels.push(this.xaxisLabels[i2]);
		...
		if (ylabels.show) for (let i2 = 0; i2 <= labels.length - 1; i2++) {
	...
	*/
	...
	getXAxisTicksPositions() {
		...
		if (w.labelData.timescaleLabels.length > 0) for (let i2 = 0; i2 < xCount; i2++) {
	...
	xAxisLabelCorrections() {
		...
		if (w.layout.rotateXLabels || w.config.xaxis.labels.rotateAlways) for (let xat = 0; xat < xAxisTexts.length; xat++) {
			...
			xAxis == null || xAxis.setAttribute("transform", `translate(0, -10)`);
	...
	*/
	...
	_drawGridLines({ i: i2, x1, y1, x2, y2, xCount, parent }) {
		...
		const shouldDraw = () => {
			...
			if (i2 === xCount - 1 && w.globals.skipLastTimelinelabel && !w.config.xaxis.labels.formatter) return false;
		...
		if (shouldDraw()) {
			...
			if (w.labelData.hasXaxisGroups && w.config.xaxis.tickPlacement === "between") {
				...
				if (groups) {
					let gacc = 0;
					for (let gi = 0; gacc < i2 && gi < groups.length; gi++) gacc += groups[gi].cols;
					if (gacc === i2) y_2 = w.layout.xAxisLabelsHeight * .6;
				}
			}
	...
	drawGridBands(xCount, tickAmount) {
		...
		const drawBands = (type, count, x1, y1, x2, y2) => {
			for (let i2 = 0, c2 = 0; i2 < count; i2++, c2++) {
				if (c2 >= w.config.grid[type].colors.length) c2 = 0;
		...
		if (((_b = w.config.grid.column.colors) == null ? void 0 : _b.length) > 0) {
			...
			for (let i2 = 0, c2 = 0; i2 < xCount; i2++, c2++) {
				if (c2 >= w.config.grid.column.colors.length) c2 = 0;
				if (w.config.xaxis.type === "datetime") {
					x1 = this.xaxisLabels[i2].position;
					x2 = (((_e = this.xaxisLabels[i2 + 1]) == null ? void 0 : _e.position) || w.layout.gridWidth) - this.xaxisLabels[i2].position;
				}
				this._drawGridBandRect({
					c: c2,
					x1,
...
var Scales = class {
	...
	*/
	_thinToTickAmount(values, tickAmount) {
		if (tickAmount === null || !Utils$1.isNumber(tickAmount) || tickAmount < 1) return values;
		...
		for (let stride = 1; stride <= intervals; stride++) {
			...
			if (best === null || count > best.count) best = {
				stride,
				count
			};
		}
	...
	*/
	logarithmicScaleNice(yMin, yMax, base, tickAmount = null) {
		...
		for (let i2 = logMin; i2 < logMax; i2++) logs.push(Math.pow(base, i2));
	...
	*/
	_logDomainSpan(yMin, yMax, base) {
		...
		return Math.abs(Math.log(yMax) / Math.log(base) - Math.log(yMin) / Math.log(base));
	}
	/**
	* @param {number} yMin
	* @param {number} yMax
	...
	*/
	logarithmicScale(yMin, yMax, base, tickAmount = null) {
		...
		for (let i2 = 0, logTick = logMin; i2 < ticks; i2++, logTick += logTickSpacing) logs.push(Math.pow(base, logTick));
	...
	*/
	setYScaleForIndex(index, minY, maxY) {
		...
		if (y.logarithmic && !validLogScale) gl.invalidLogScale = true;
		if (validLogScale) {
			gl.allSeriesCollapsed = false;
	...
	scaleMultipleYAxes() {
			...
			if (axisSeries.length > 0) {
				...
				if (cnf.chart.stacked) {
					...
					for (let i2 = 0; i2 < axisSeries.length; i2++) {
						if (!seriesType && cnf.series[axisSeries[i2]].type) seriesType = cnf.series[axisSeries[i2]].type;
				...
				} else for (let i2 = 0; i2 < axisSeries.length; i2++) {
					const si = axisSeries[i2];
		...
		if (alignZeroParticipants.length >= 2) {
			...
			alignZeroParticipants.forEach((p) => {
				...
				const extendMin = r2 < targetRatio && targetRatio < .999999999;
...
var Range = class {
	...
	*/
	_autoScaleYEnabled() {
		var _a;
		const cnf = this.w.config;
		const brush = (_a = this.w.globals.brushSource) == null ? void 0 : _a.w.config.chart.brush;
		return !!(cnf.chart.zoom.enabled && cnf.chart.zoom.autoScaleYaxis || (brush == null ? void 0 : brush.enabled) && (brush == null ? void 0 : brush.autoScaleYaxis));
	}
	/**
	...
	*/
	_autoScaleXBounds(i2) {
		const cnfX = this.w.config.xaxis;
		if (!cnfX.min && !cnfX.max) return null;
		const tolerance = this._xPixelTolerance(this.w.seriesData.seriesX[i2]);
		return {
	...
	*/
	getMinYMaxY(startingSeriesIndex, lowestY = Number.MAX_VALUE, highestY = -Number.MAX_VALUE, endingSeriesIndex = null) {
		...
		if (this.w.seriesData.seriesX.length >= endingSeriesIndex) {
			if (this._autoScaleYEnabled() && !this._ignoreAutoScaleWindow) autoScaleYaxis = true;
		}
		for (let i2 = startingSeriesIndex; i2 < endingSeriesIndex; i2++) {
			...
			if (this.w.labelData.labels.length && cnf.xaxis.type !== "datetime" && this.w.seriesData.series.reduce((a2, c2) => a2 + c2.length, 0) !== 0) gl.dataPoints = Math.max(gl.dataPoints, this.w.labelData.labels.length);
			let firstXIndex = 0;
			let lastXIndex = series[i2].length - 1;
			if (autoScaleYaxis) {
				const xs = this.w.seriesData.seriesX[i2];
				const bounds = this._autoScaleXBounds(i2);
				if (bounds && cnf.xaxis.min) for (; firstXIndex < lastXIndex && xs[firstXIndex] < bounds.lo; firstXIndex++);
				if (bounds && cnf.xaxis.max) for (; lastXIndex > firstXIndex && xs[lastXIndex] > bounds.hi; lastXIndex--);
				if (bounds && xs && xs.length) {
					if (!(xs[firstXIndex] >= bounds.lo && xs[firstXIndex] <= bounds.hi)) if (xs[0] < bounds.lo && xs[xs.length - 1] > bounds.hi && firstXIndex > 0) {
			...
			if (seriesMin === series && seriesMax === series && cnf.chart.type !== "boxPlot" && seriesType !== "candlestick" && seriesType !== "boxPlot" && seriesType !== "violin" && seriesType !== "rangeArea" && seriesType !== "rangeBar" && !(this.w.seriesData.seriesGoals[i2] && this.w.seriesData.seriesGoals[i2].length)) {
				const arr = series[i2];
						...
						if (val !== null && typeof val === "number" && val === val && val !== Infinity && val !== -Infinity) {
							...
							if (!Number.isInteger(val)) {
								...
								if (av >= 1e-6 && av < 1e21) {
									...
									if (Utils$1.isFloat(nv)) yDec = Math.max(yDec, nv.toString().split(".")[1].length);
								}
							}
						} else hasNulls = true;
					}
				...
				if (val !== null && Utils$1.isNumber(val)) {
					...
					switch (seriesType) {
						...
						case "boxPlot":
							...
							break;
						case "violin":
							if (typeof ((_d = this.w.violinData.seriesViolinMax[i2]) == null ? void 0 : _d[j]) !== "undefined") {
								maxY = Math.max(maxY, this.w.violinData.seriesViolinMax[i2][j]);
								lowestY = Math.min(lowestY, this.w.violinData.seriesViolinMin[i2][j]);
							}
							break;
					}
					...
					if (this.w.seriesData.seriesGoals[i2] && this.w.seriesData.seriesGoals[i2][j] && Array.isArray(this.w.seriesData.seriesGoals[i2][j])) this.w.seriesData.seriesGoals[i2][j].forEach((g) => {
						maxY = Math.max(maxY, g.value);
						lowestY = Math.min(lowestY, g.value);
					});
					if (this.w.config.chart.type === "boxPlot" || seriesType === "boxPlot") {
						const boxPts = (_f = (_e = this.w.candleData.seriesBoxPoints) == null ? void 0 : _e[i2]) == null ? void 0 : _f[j];
					...
					if (minY > ((_g = seriesMin[i2]) == null ? void 0 : _g[j]) && ((_h = seriesMin[i2]) == null ? void 0 : _h[j]) < 0) minY = seriesMin[i2][j];
		...
		if (droppedForWindow && maxY === -Number.MAX_VALUE) {
			...
			try {
				return this.getMinYMaxY(startingSeriesIndex, Number.MAX_VALUE, -Number.MAX_VALUE, endingSeriesIndex);
			} finally {
	...
	setXRange() {
		...
		const getInitialMinXMaxX = () => {
			...
			let maxX = gl.maxX;
			for (let i2 = 0; i2 < this.w.seriesData.series.length; i2++) {
				const lbls = this.w.labelData.labels[i2];
				if (!lbls) continue;
				const pe = (_a = this.w.seriesData._parsedExtrema) == null ? void 0 : _a[i2];
		...
		if (this.w.axisFlags.isXNumeric || this.w.axisFlags.noLabelsProvided || this.w.axisFlags.dataFormatXNumeric) {
			...
			if (gl.minX !== Number.MAX_VALUE && gl.maxX !== -Number.MAX_VALUE) if (cnf.xaxis.convertedCatToNumeric && !this.w.axisFlags.dataFormatXNumeric) {
				const catScale = [];
				for (let i2 = gl.minX - 1; i2 < gl.maxX; i2++) catScale.push(i2 + 1);
				gl.xAxisScale = {
					result: catScale,
					niceMin: catScale[0],
	...
	setZRange() {
		...
		for (let i2 = 0; i2 < this.w.seriesData.series.length; i2++) if (typeof this.w.seriesData.seriesZ[i2] !== "undefined") {
			for (let j = 0; j < this.w.seriesData.seriesZ[i2].length; j++) if (this.w.seriesData.seriesZ[i2][j] !== null && Utils$1.isNumber(this.w.seriesData.seriesZ[i2][j])) {
		...
		if (Utils$1.isNumber(bubbleCfg.minZ) && bubbleCfg.minZ < gl.minZ) gl.minZ = bubbleCfg.minZ;
		if (Utils$1.isNumber(bubbleCfg.maxZ) && bubbleCfg.maxZ > gl.maxZ) gl.maxZ = bubbleCfg.maxZ;
	}
	...
	_getMinXDiff() {
		const gl = this.w.globals;
		if (this.w.axisFlags.isXNumeric) this.w.seriesData.seriesX.forEach((sX, si) => {
			var _a;
			if (sX.length) {
	...
	_setStackedMinMax() {
		...
		seriesGroups.forEach((group) => {
			stackedPoss[group] = [];
			stackedNegs[group] = [];
			this.w.config.series.map((serie, si) => group.indexOf(this.w.seriesData.seriesNames[si]) > -1 ? si : null).filter((f) => f !== null).forEach((i2) => {
				var _a, _b, _c, _d;
				const bounds = windowed ? this._autoScaleXBounds(i2) : null;
				const xs = this.w.seriesData.seriesX[i2];
				...
				for (let j = 0; j < this.w.seriesData.series[gl.maxValsInArrayIndex].length; j++) {
					...
					if (this.w.config.chart.stacked && !gl.comboCharts || this.w.config.chart.stacked && gl.comboCharts && (!this.w.config.chart.stackOnlyBar || ((_b = (_a = this.w.config.series) == null ? void 0 : _a[i2]) == null ? void 0 : _b.type) === "bar" || ((_d = (_c = this.w.config.series) == null ? void 0 : _c[i2]) == null ? void 0 : _d.type) === "column")) {
						if (this.w.seriesData.series[i2][j] !== null && Utils$1.isNumber(this.w.seriesData.series[i2][j])) this.w.seriesData.series[i2][j] > 0 ? stackedPoss[group][j] += parseFloat(String(this.w.seriesData.series[i2][j])) + 1e-4 : stackedNegs[group][j] += parseFloat(String(this.w.seriesData.series[i2][j]));
					}
				}
			});
		});
...
var YAxis = class {
	...
	*/
	drawYaxis(realIndex) {
		...
		if (w.config.yaxis[realIndex].labels.show) {
			...
			let firstLabel = null;
			for (let i2 = tickAmount; i2 >= 0; i2--) {
				const val = labelStep > 1 && i2 % labelStep !== 0 ? "" : lbFormatter(labels[i2], i2, w);
	...
	*/
	drawYaxisInversed(realIndex) {
		...
		if (w.config.xaxis.labels.show) for (let i2 = timescaleLabels.length ? 0 : tickAmount; timescaleLabels.length ? i2 < timescaleLabels.length : i2 >= 0; timescaleLabels.length ? i2++ : i2--) {
			...
			if (timescaleLabels.length) {
				const label = this.axesUtils.getLabel(labels, timescaleLabels, x, i2, this.drawnLabels, this.xaxisFontSize);
				x = label.x;
				val = label.text;
				this.drawnLabels.push(label.text);
				if (i2 === 0 && w.globals.skipFirstTimelinelabel) val = "";
				if (i2 === labels.length - 1 && w.globals.skipLastTimelinelabel) val = "";
			}
...
var Events = class {
	...
	*/
	fireEvent(name2, args) {
		...
		const l2 = evs.length;
		for (let i2 = 0; i2 < l2; i2++) evs[i2].apply(null, args);
	}
	setupEventHandlers() {
		const w = this.w;
		const me = this.ctx;
		...
		this.ctx.eventList.forEach((event) => {
			clickableArea == null || clickableArea.addEventListener(event, (e2) => {
				...
				if (e2.type === "keydown") {
					if (w.config.chart.accessibility.enabled && w.config.chart.accessibility.keyboard.enabled) {
						if (me.ctx.keyboardNavigation) me.ctx.keyboardNavigation.handleKey(e2);
						if (typeof w.config.chart.events.keyDown === "function") w.config.chart.events.keyDown(e2, me, opts);
					...
					if (w.config.chart.accessibility.enabled && w.config.chart.accessibility.keyboard.enabled) {
						if (typeof w.config.chart.events.keyUp === "function") w.config.chart.events.keyUp(e2, me, opts);
					...
					if (typeof w.config.chart.events.mouseMove === "function") w.config.chart.events.mouseMove(e2, me, opts);
				} else if (e2.type === "mouseleave" || e2.type === "touchleave") {
					if (typeof w.config.chart.events.mouseLeave === "function") w.config.chart.events.mouseLeave(e2, me, opts);
				} else if (e2.type === "mouseup" && e2.which === 1 || e2.type === "touchend") {
					if (typeof w.config.chart.events.click === "function") w.config.chart.events.click(e2, me, opts);
...
var Crosshairs = class {
	...
	*/
	...
	drawYCrosshairs() {
		const w = this.w;
		w.dom.elGraphical.node.querySelectorAll(":scope > .apexcharts-ycrosshairs, :scope > .apexcharts-ycrosshairs-hidden").forEach((el) => el.remove());
		const graphics = new Graphics(this.w);
...
var Responsive = class {
	...
	*/
	checkResponsiveConfig(opts) {
		...
		res.sort((a2, b) => a2.breakpoint > b.breakpoint ? 1 : b.breakpoint > a2.breakpoint ? -1 : 0).reverse();
		...
		const iterateResponsiveOptions = (newOptions = {}) => {
			...
			} else for (let i2 = 0; i2 < res.length; i2++) if (width < res[i2].breakpoint) {
...
function captureStreamFrame(w) {
	...
	const rPixels = [];
	if ((_b = (_a = w.dom) == null ? void 0 : _a.baseEl) == null ? void 0 : _b.querySelectorAll) w.dom.baseEl.querySelectorAll(".apexcharts-marker").forEach((node) => {
		var _a2, _b2, _c2, _d, _e;
		const ri = parseInt((_a2 = node.getAttribute("index")) != null ? _a2 : "", 10);
		const j = parseInt((_c2 = (_b2 = node.getAttribute("j")) != null ? _b2 : node.getAttribute("rel")) != null ? _c2 : "", 10);
		const r2 = parseFloat((_e = (_d = node.getAttribute("r")) != null ? _d : node.getAttribute("default-marker-size")) != null ? _e : "");
		if (isFinite(ri) && isFinite(j) && isFinite(r2)) (rPixels[ri] = rPixels[ri] || [])[j] = r2;
	});
	gl.prevStreamFrame = {
...
function joinKeys(oldKeys, newKeys) {
	...
	const exits = [];
	for (let i2 = 0; i2 < oldKeys.length; i2++) if (!usedOld.has(i2)) exits.push(i2);
	return {
		toOld,
		exits,
		ordered,
...
function grabLines(root, sel, posAttr) {
	return [...root.querySelectorAll(sel)].map((el) => parseFloat(el.getAttribute(posAttr) || ""));
}
var NO_GHOST = ":not(.apexcharts-tick-ghost)";
var X_LABELS_SEL = `.apexcharts-xaxis-texts-g text:not(.apexcharts-xaxis-group-label)${NO_GHOST}`;
var Y_LABELS_SEL = `.apexcharts-yaxis-texts-g text${NO_GHOST}`;
var V_GRID_SEL = `.apexcharts-gridlines-vertical line${NO_GHOST}`;
var H_GRID_SEL = `.apexcharts-gridlines-horizontal line${NO_GHOST}`;
function currentXScale(w) {
	...
	if (!isFinite(min) || !isFinite(max) || !(max > min) || !(width > 0)) return null;
	return {
		min,
		max,
		width
...
function fadeIn$1(w, node, duration, ease) {
	...
	style.opacity = "0";
	rafTween(w, duration, ease, (eased) => {
...
var MAX_GHOSTS = 20;
function transitionAxis(w, { newLabels, oldLabels, posAttr, newLines, oldLines, lineAttrs, duration, ease, project }) {
	...
	const spanPs = oldLabels.map((l2) => l2.pos).concat(newLabels.map((l2) => parseFloat(l2.getAttribute(posAttr) || ""))).filter((p) => isFinite(p));
	const spanLo = Math.min(...spanPs);
	const spanHi = Math.max(...spanPs);
	const margin = Math.max(40, (spanHi - spanLo) * .25);
	...
	const tweenPairedLine = (line, old) => {
		...
		if (isFinite(lineTo) && isFinite(lineFrom)) tweenPos(w, line, lineAttrs, lineFrom, lineTo, duration, ease);
	};
	newLabels.forEach((label, i2) => {
		...
		if (!old || !isFinite(old.pos)) {
			if (!old) {
				if (project && isFinite(to) && !labelTransform) {
					...
					if (isFinite(from) && Math.abs(from - to) > .5) {
		...
		if (!isFinite(to) || Math.abs(old.pos - to) < .5) return;
	...
	oldLabels.forEach((old, i2) => {
		...
		if (!isFinite(rawTo) || Math.abs(rawTo - old.pos) < .5) return;
		ghosts++;
		spawnGhost(w, {
		...
		if (oldLinesAligned && newLines.length && isFinite(oldLines[i2])) spawnGhost(w, {
			template: newLines[0],
			attrs: lineAttrs,
			from: oldLines[i2],
...
function applyAxisTransition(w) {
	...
	if (!(w.seriesData.series || []).some((_, i2) => seriesJoin(w, i2, true, true) !== null)) return;
	const root = w.dom.baseEl;
	if (!Utils$1.elementExists(root)) return;
	...
	try {
		...
		const projY = composeYMap(chrome.yAnchors, currentYAnchors(w, newYLabels.map((el) => ({ pos: parseFloat(el.getAttribute("y") || "") }))));
		transitionAxis(w, {
...
var DL_GROUP_SEL = ".apexcharts-data-labels[data\\:dlKey]";
var DL_TEXT_SEL = ".apexcharts-datalabel";
var DL_TOTAL_SEL = ".apexcharts-datalabel-total[data\\:dlTotalKey]";
...
function rideTo(w, { el, oldCx, oldCy, duration, ease, delay = 0 }) {
	...
	const start = () => rafTween(w, duration, ease, (eased) => {
		const t2 = 1 - eased;
...
function applyDataLabelTransition(w) {
	...
	try {
		root.querySelectorAll(DL_GROUP_SEL).forEach((group) => {
			...
			if (ride) {
				if (old && isFinite(old.cx) && isFinite(old.cy)) rideTo(w, {
					el: group,
					oldCx: old.cx,
					oldCy: old.cy,
					duration,
				...
				else if (!old) fadeIn(w, group, duration, ease);
			}
			if (countUp && old) {
		...
		root.querySelectorAll(DL_TOTAL_SEL).forEach((el) => {
			...
			if (ride && isFinite(old.cx) && isFinite(old.cy)) rideTo(w, {
				el,
				oldCx: old.cx,
				oldCy: old.cy,
				duration,
			...
			if (countUp) {
				const realIndex = parseInt(el.getAttribute("data:dlTotalSeries") || key, 10);
				countUpText(w, {
					el,
					from: old.val,
...
var Series = class Series {
	...
	*/
	static addCollapsedClassToSeries(w, elSeries, index) {
		...
		function iterateOnAllCollapsedSeries(series) {
			for (let cs = 0; cs < series.length; cs++) if (series[cs].index === index) {
	...
	*/
	...
	resetSeries(shouldUpdateChart = true, shouldResetZoom = true, shouldResetCollapsed = true) {
		...
		if (!Array.isArray(series)) series = Utils$1.clone(w.config.series) || [];
		w.globals.previousPaths = [];
	...
	*/
	emptyCollapsedSeries(series) {
		...
		for (let i2 = 0; i2 < series.length; i2++) if (w.globals.collapsedSeriesIndices.indexOf(i2) > -1) if (series[i2] && typeof series[i2] === "object") series[i2].data = [];
		else series[i2] = 0;
		return series;
	}
	/**
	...
	*/
	canvasRestyle(dim) {
		...
		if (r2 && r2.kind === "canvas" && typeof r2.restyle === "function") r2.restyle(dim);
	}
	/**
	* @param {string} seriesName
	*/
	highlightSeries(seriesName) {
		...
		if (seriesEl) {
			if (!w.globals.axisCharts) {
				const parentEl = seriesEl.parentNode;
				parentEl == null || parentEl.classList.remove(this.legendInactiveClass);
			}
		...
		this.canvasRestyle(seriesEl && !Number.isNaN(realIndex) ? {
			active: realIndex,
			opacity: .2
		} : null);
	}
	...
	*/
	toggleSeriesOnHover(e2, targetElement) {
		...
		if (e2.type === "mousemove") {
			...
			for (let se = 0; se < allSeriesEls.length; se++) allSeriesEls[se].classList.remove(this.legendInactiveClass);
	...
	highlightRangeInSeries(rangeIndex, action) {
		...
		const toggleAllInactive = (op) => {
			for (let i2 = 0; i2 < allHeatMapElements.length; i2++) {
				const classList = allHeatMapElements[i2].classList;
		...
		for (let i2 = 0; i2 < allHeatMapElements.length; i2++) {
			...
			if (val >= range.from && val <= range.to) allHeatMapElements[i2].classList.remove(this.legendInactiveClass);
		}
	}
	/**
	...
	getActiveConfigSeriesIndex(order = "asc", chartTypes = []) {
		...
		if (w.config.series.length > 1) {
			const activeSeriesIndex = w.config.series.map((s2, index) => {
				...
				return s2.data && s2.data.length > 0 && w.globals.collapsedSeriesIndices.indexOf(index) === -1 && checkChartType() ? index : -1;
			});
			for (let a2 = order === "asc" ? 0 : activeSeriesIndex.length - 1; order === "asc" ? a2 < activeSeriesIndex.length : a2 >= 0; order === "asc" ? a2++ : a2--) if (activeSeriesIndex[a2] !== -1) {
	...
	*/
	getActiveConfigSeriesIndexByGroup(chartTypes = []) {
		const w = this.w;
		return (w.labelData.seriesGroups || []).map((group) => {
			let last = -1;
			w.config.series.forEach((s2, i2) => {
				if (group.indexOf(w.seriesData.seriesNames[i2]) === -1) return;
				if (w.globals.comboCharts && chartTypes.length && chartTypes.indexOf(s2.type) === -1) return;
				if (s2.data && s2.data.length > 0 && w.globals.collapsedSeriesIndices.indexOf(i2) === -1) last = i2;
			});
			return last;
		});
	}
	getBarSeriesIndices() {
		if (this.w.globals.comboCharts) return this.w.config.series.map((s2, i2) => {
	...
	getPreviousPaths() {
		...
		if (heatTreeSeries.length > 0) for (let h2 = 0; h2 < heatTreeSeries.length; h2++) {
			const seriesEls = w.dom.baseEl.querySelectorAll(`.apexcharts-${w.config.chart.type} .apexcharts-series[data\\:realIndex='${h2}'] rect`);
			const dArr = [];
	...
	*/
	...
	hasAllSeriesEqualX() {
		...
		for (let i2 = 0; i2 < filteredSerX.length - 1; i2++) if (filteredSerX[i2][0] !== filteredSerX[i2 + 1][0]) {
			equalLen = false;
			break;
		}
...
function readTokens(w) {
	...
	const read = (name2) => {
		const v = cs.getPropertyValue(name2);
...
var THEME_KEY = "__apexcharts_themes__";
if (!globalThis[THEME_KEY]) globalThis[THEME_KEY] = {};
function getThemes() {
...
function registerTheme(name2, def) {
	...
	if (def != null && (typeof def !== "object" || Array.isArray(def))) {
		console.warn(`ApexCharts: registerTheme("${name2}") expects an object like { mode, palette, tokens, monochrome, accessibility }.`);
		return;
	}
	getThemes()[name2] = def || {};
}
...
var DEFAULT_FORECOLOR_LIGHT = "#373d3f";
var DEFAULT_FORECOLOR_DARK = "#f6f7f8";
var DEFAULT_AXIS_GRID = "#e0e0e0";
var Theme = class {
	/**
	* @param {import('../types/internal').ChartStateW} w
	*/
	...
	*/
	_applyModeDefaults() {
		...
		if (mode === "dark") {
			if (w.config.chart.foreColor === DEFAULT_FORECOLOR_LIGHT) w.config.chart.foreColor = DEFAULT_FORECOLOR_DARK;
			if (w.config.theme.palette === "palette1") w.config.theme.palette = "palette4";
			if (w.config.tooltip && w.config.tooltip.theme !== "light") w.config.tooltip.theme = "dark";
		} else if (mode === "light") {
			if (w.config.chart.foreColor === DEFAULT_FORECOLOR_DARK) w.config.chart.foreColor = DEFAULT_FORECOLOR_LIGHT;
			if (w.config.theme.palette === "palette4") w.config.theme.palette = "palette1";
		}
	}
	/**
	...
	*/
	_applyNamedThemeMode() {
		...
		if (named.mode && !theme.mode) theme.mode = named.mode;
		...
		if (named.monochrome && named.monochrome.enabled && !theme.monochrome.enabled) theme.monochrome = __spreadValues(__spreadValues({}, theme.monochrome), named.monochrome);
	}
	/**
	...
	*/
	applyTokenChrome(tokens) {
		if (!tokens) return;
		const w = this.w;
		if (tokens.fore && (w.config.chart.foreColor === DEFAULT_FORECOLOR_LIGHT || w.config.chart.foreColor === DEFAULT_FORECOLOR_DARK)) w.config.chart.foreColor = tokens.fore;
		if (tokens.grid) {
			if (w.config.grid.borderColor === DEFAULT_AXIS_GRID) w.config.grid.borderColor = tokens.grid;
			const applyAxis = (axis) => {
				if (!axis) return;
				if (axis.axisBorder && axis.axisBorder.color === DEFAULT_AXIS_GRID) axis.axisBorder.color = tokens.grid;
				if (axis.axisTicks && axis.axisTicks.color === DEFAULT_AXIS_GRID) axis.axisTicks.color = tokens.grid;
			};
			applyAxis(w.config.xaxis);
			if (Array.isArray(w.config.yaxis)) w.config.yaxis.forEach(applyAxis);
			else applyAxis(w.config.yaxis);
		}
		...
		if (tokens.surface) {
			if (isOurs) {
				...
				if (paperNode && paperNode.style) paperNode.style.background = tokens.surface;
			}
		} else if (appliedSurface && currentBg === appliedSurface) {
			w.config.chart.background = "";
			w.globals.tokenSurface = void 0;
			const paperNode = w.dom.Paper && w.dom.Paper.node;
			if (paperNode && paperNode.style) paperNode.style.background = "";
		}
	}
	/**
	...
	*/
	pushExtraColors(colorSeries, length, distributed = null) {
		...
		if (colorSeries.length < len) {
			...
			for (let i2 = 0; i2 < diff; i2++) colorSeries.push(colorSeries[i2]);
		}
	}
	/**
	...
	*/
	...
	predefined() {
		...
		const palettes = getThemePalettes();
		const builtin = palettes[palette] || palettes.palette1;
		const tokens = this._tokens || {};
		if (Array.isArray(tokens.series) && tokens.series.length) return tokens.series.slice();
		if (tokens.accent) return [tokens.accent, ...builtin];
		const named = this._namedTheme;
		if (named && Array.isArray(named.palette) && named.palette.length) return named.palette.slice();
		return builtin;
	}
};
...
var Helpers = class {
	...
	**/
	getSparklineStrokeInset() {
		...
		if (!w.config.stroke.show || !(half > 0)) return {
			top: 0,
			bottom: 0
		};
		const yRange = w.globals.maxY - w.globals.minY;
		const extremes = this._getSeriesYExtremes();
		if (!this._strokeTracesDataPoints() || !(yRange > 0) || !extremes) return {
			top: half,
			bottom: half
		};
	...
	**/
	_strokeTracesDataPoints() {
		...
		return w.config.series.every((s2) => tracesPoints.includes(s2.type || w.config.chart.type));
	}
	/**
	...
	**/
	getxAxisLabelsCoords() {
		...
		if (w.labelData.timescaleLabels.length > 0) {
			...
			if (w.globals.isBarHorizontal) {
				val = w.globals.yAxisScale[0].result.reduce((a2, b) => a2.length > b.length ? a2 : b, 0);
				valArr = val;
			}
			const xFormat = new Formatters(this.w);
			const timestamp = val;
	...
	getxAxisTimeScaleLabelsCoords() {
		...
		const val = labels.reduce((a2, b) => {
			if (typeof a2 === "undefined") {
				...
				return 0;
			} else return a2.length > b.length ? a2 : b;
		}, 0);
...
var DimGrid = class {
	...
	*/
	gridPadForStackedTotalDataLabels() {
		...
		if (!w.globals.isBarHorizontal || !w.config.chart.stacked || w.config.chart.stackType !== "100%" || !totalConfig.enabled) return;
		const totals = w.seriesData.stackedSeriesTotals || [];
		if (!totals.length) return;
		const formatter = totalConfig.formatter || w.config.dataLabels.formatter;
		const labels = totals.map((val, j) => String(formatter ? formatter(val, __spreadProps(__spreadValues({}, w), {
			seriesIndex: 0,
			dataPointIndex: j,
			w
		})) : val));
		const needed = new Graphics(w).getTextRects(Utils$1.getLargestStringFromArr(labels), parseFloat(totalConfig.style.fontSize).toString(), totalConfig.style.fontFamily).width + Math.abs(totalConfig.offsetX || 0) + 2;
	...
	gridPadFortitleSubtitle() {
		...
		["title", "subtitle"].forEach((t2) => {
			if (w.config[t2].text !== void 0) gridShrinkOffset += w.config[t2].margin;
...
function breadcrumbConfig(w, localCfg) {
	return __spreadValues(__spreadValues({
		...
		offsetY: 0,
		formatter: void 0
	}, w.config.drilldown && w.config.drilldown.breadcrumb || {}), localCfg || {});
}
var Dimensions = class {
	...
	**/
	plotCoords() {
		...
		if (this.isSparkline) {
			if (this.w.globals.markers.largestSize > 0) Object.entries(this.gridPad).forEach(([k, v]) => {
	...
	*/
	gridPadForBreadcrumb() {
		...
		const needed = 24 + (isTreemap ? 0 : Math.ceil(labelFs * LINE_HEIGHT_RATIO / 2));
	...
	setDimensionsForNonAxisCharts() {
		...
		if (cnf.chart.type === "unit") {
			...
			let top = 0;
			let side = 0;
			if (legendVisible) if (pos === "bottom" || pos === "top") top = this.lgRect.height;
			else side = this.lgRect.width + xPad;
...
var REGISTRY_KEY$1 = "__apexcharts_registry__";
var CUSTOM_KEY = "__apexcharts_custom_types__";
if (!globalThis[REGISTRY_KEY$1]) globalThis[REGISTRY_KEY$1] = {};
if (!globalThis[CUSTOM_KEY]) globalThis[CUSTOM_KEY] = /* @__PURE__ */ new Set();
function getRegistry$1() {
	return globalThis[REGISTRY_KEY$1];
}
function getCustomTypes() {
	return globalThis[CUSTOM_KEY];
}
...
function getChartClass(type) {
	...
	if (!Cls) throw new Error(`ApexCharts: chart type "${type}" is not registered. Bundler: import 'apexcharts/${type}'. Script tag: add <script src=".../dist/${type}.js"> after apexcharts.core.js, or load the full apexcharts.js instead.`);
	return Cls;
}
var Core = class {
	/**
	...
	*/
	...
	setupElements() {
		...
		const isCustomType = !axisChartsArrTypes.includes(ct) && ![
			"pie",
			"donut",
			"polarArea",
			"radialBar"
		].includes(ct) && isCustom(ct);
		...
		this.w.dom.elLegendForeign = BrowserAPIs.createElementNS(SVGNS$1, "foreignObject");
	...
	_classifySeriesByType(ser) {
		...
		this.w.seriesData.series.forEach((serie, st) => {
			...
			if (st_[seriesType]) {
				...
				if (!customBuckets[seriesType]) customBuckets[seriesType] = {
					series: [],
					i: []
				};
	...
	*/
	plotChartType(ser, xyRatios) {
		...
		const elGraph = this._assembleSeriesGraphics(seriesTypes, customBuckets, renderers, xyRatios);
		if (canvasMode) {
			const rr = ctx.renderer;
			if (rr && rr._repaintHostInPlace) {
				rr._repaintHostInPlace = false;
				const wrapNode = this.w.dom.elGraphical.node.querySelector(".apexcharts-canvas-series-wrap");
				if (wrapNode && rr.canRepaintInPlace && rr.canRepaintInPlace()) {
					rr.repaintInPlace();
					(Array.isArray(elGraph) ? elGraph : [elGraph]).forEach((g) => {
			...
			if (host) {
				const wrap = new Graphics(w).group({ class: "apexcharts-canvas-series-wrap" });
				wrap.add(host);
				(Array.isArray(elGraph) ? elGraph : [elGraph]).forEach((g) => {
					if (g) wrap.add(g);
				});
		...
		if (gl.comboCharts) {
			...
			switch (type) {
				...
				case "treemap":
					...
					break;
				case "unit":
					elGraph = new (getChartClass("unit"))(ctx.w, ctx).draw(this.w.seriesData.series);
					break;
				case "sunburst":
					elGraph = new (getChartClass("sunburst"))(ctx.w, ctx).draw(this.w.seriesData.series);
					break;
				case "pie":
				case "donut":
				case "polarArea":
				...
				case "radar":
					...
					break;
				default: if (isCustom(type)) elGraph = new (getChartClass(type))(ctx.w, ctx, xyRatios).draw(this.w.seriesData.series, type);
				else elGraph = line.draw(this.w.seriesData.series);
			}
		}
		return elGraph;
	}
	...
	*/
	getResizeSignature() {
		...
		if (rawHeight.endsWith("%")) h2 = Utils$1.getDimensions(this.el.parentNode)[1] || 0;
		const sig = {
			w: Math.round(w),
			h: Math.round(h2)
		};
		if (cnf.responsive && cnf.responsive.length && Environment.isBrowser()) sig.iw = window.innerWidth;
		return sig;
	}
	shiftGraphPosition() {
	...
	resizeNonAxisCharts() {
		...
		const el = w.dom.baseEl.querySelector(".apexcharts-radialbar, .apexcharts-pie, .apexcharts-sunburst");
		...
		const radialAngleSpan = Math.abs(w.config.plotOptions[angleType].endAngle - w.config.plotOptions[angleType].startAngle);
		if (el && !w.config.chart.sparkline.enabled && radialAngleSpan < 360) {
			...
			Array.from((_c = el.children) != null ? _c : []).forEach((c2) => accumulate(
				/** @type {Element} */
				c2
			));
			...
			if (!isPercentHeight) {
				...
				if (Environment.isBrowser()) {
					...
					if (grandparent) grandparent.style.minHeight = `${elWrapHeight}px`;
				}
				w.globals.svgHeight = svgHeight;
				if (w.config.legend.position === "bottom" && w.config.legend.show && !w.config.legend.floating) (_h = this.ctx.legend) == null || _h.setLegendWrapXY(20, 0);
			}
			return;
		}
		...
		if (Environment.isBrowser()) {
			...
			if (grandparent) grandparent.style.minHeight = `${newHeight}px`;
		}
	}
	coreCalculations() {
	...
	setupBrushHandler() {
		...
		if (typeof w.config.chart.events.selection !== "function") {
			...
			targets.forEach((target) => {
				...
				if (!targetChart) {
					console.warn(`ApexCharts: brush target "${target}" was not found. Ensure the target chart is rendered (and its chart.id matches) before the brush chart.`);
					return;
				}
			...
			w.config.chart.events.selection = (chart, e2) => {
				targets.forEach((target) => {
					...
					targetChart.ctx.updateHelpers._updateOptions({ xaxis: {
						min: e2.xaxis.min,
						max: e2.xaxis.max
	...
	getAccessibleChartLabel() {
		...
		if (cnf.title.text) {
			...
			const namedSeries = (() => {
				...
				if (Array.isArray(cnf.series)) return cnf.series.map((s2) => typeof s2 === "object" && s2 !== null ? s2.name : null).filter(Boolean);
				return [];
			})();
...
function drilldownById(w, id) {
	...
	return (dd && Array.isArray(dd.series) ? dd.series : []).find((s2) => s2 && s2.id === id);
}
function toNode(w, d, i2, paletteFromParent, parentKey, seenIds = null, opts = {}) {
	...
	if (paletteFromParent && !node.color) node.color = paletteFromParent[i2 % paletteFromParent.length];
	if (opts.keepDatum) node._datum = d;
	if (isObj && Array.isArray(d.children) && d.children.length) node.children = d.children.map((c2, j) => toNode(w, c2, j, null, node._key, seenIds, opts));
		...
		if (!visited.has(d.drilldown)) {
			...
			if (dd && Array.isArray(dd.data) && dd.data.length) {
				...
				node.children = dd.data.map((c2, j) => toNode(w, c2, j, palette, node._key, nextSeen, opts));
			}
		}
	}
	return node;
...
function buildSeriesRoots(w, series, opts = {}) {
	const cfgSeries = series || w.config.series;
	...
	return cfgSeries.map((s2, i2) => {
		...
		return {
			...
			_key: key,
			_seriesIndex: i2,
			children: data.map((d, j) => toNode(w, d, j, null, key, null, opts))
		};
	});
}
function fillValues(node) {
	if (node.children && node.children.length) {
		...
		if (node.value == null || isNaN(node.value)) node.value = node.children.reduce((s2, c2) => s2 + Math.max(0, c2.value || 0), 0);
	}
	...
	roots.forEach((root, si) => {
		...
		const walk = (node, depth, parent) => {
			...
			if (node._leaf) {
				...
				node._di = -1;
				node.children.forEach((c2) => walk(c2, depth + 1, node));
			}
		};
		walk(root, 0, null);
...
function leafRow(node) {
	...
	if (d && typeof d === "object") {
		const row = __spreadProps(__spreadValues({}, d), {
			x: node.name,
			y: node.value
		});
		delete row.children;
...
function resolveTreemapTree(w, series) {
	...
	return {
		roots,
		leafSeries: series.map((s2, i2) => __spreadProps(__spreadValues({}, s2), { data: (leaves[i2] || []).map(leafRow) })),
		maxDepth
	};
}
var RAW_SAMPLE_FEATURES = {
	histogram: "stats",
	raincloud: "raincloud"
};
var Data = class Data {
	...
	*/
	_fast2DArrayParse(data, i2) {
		...
		for (let j = 0; j < n2; j++) {
			...
			const x = point[0];
			const y = point[1];
			if (xNumeric) if (typeof x === "number") {
				if (x === x) {
					if (x < minX) minX = x;
					if (x > maxX) maxX = x;
				}
				...
				} else if (d < 0) xSorted = false;
				prevX = x;
			} else xNumeric = false;
			if (typeof y === "number") if (y === y && y !== Infinity && y !== -Infinity) {
				if (y > maxY) maxY = y;
	...
	*/
	handleFormat2DArray(ser, i2) {
		...
		if (!isBoxPlot && cnf.xaxis.type !== "datetime" && this._fast2DArrayParse(data, i2)) return;
		for (let j = 0; j < data.length; j++) {
			const point = data[j];
			const x = point[0];
			const y = point[1];
	...
	*/
	handleFormatXY(ser, i2) {
		...
		const data = ser[i2].data;
		let activeI = i2;
		if (gl.collapsedSeriesIndices.indexOf(i2) > -1) activeI = this.activeSeriesIndex;
		...
		for (let j = 0; j < data.length; j++) {
			...
			if (typeof this.seriesGoals[i2] === "undefined") this.seriesGoals[i2] = [];
			if (typeof point.goals !== "undefined" && Array.isArray(point.goals)) this.seriesGoals[i2].push(point.goals);
			else this.seriesGoals[i2].push(null);
		...
		for (let j = 0; j < activeData.length; j++) {
			...
			} else if (cnf.xaxis.type === "datetime") this.twoDSeriesX.push(x instanceof Date ? x.getTime() : dt.parseDate(x.toString()));
			else {
				this.w.axisFlags.dataFormatXNumeric = true;
				this.w.axisFlags.isXNumeric = true;
				this.twoDSeriesX.push(x instanceof Date ? x.getTime() : parseFloat(x));
			}
			else if (isXArr) {
				this.fallbackToCategory = true;
				this.twoDSeriesX.push(x);
	...
	*/
	handleRangeData(ser, i2) {
		...
		if (this.isFormat2DArray()) range = this.handleRangeDataFormat("array", ser, i2);
		else if (this.isFormatXY()) range = this.handleRangeDataFormat("xy", ser, i2);
	...
	handleCustomRangeData(ser, i2, yExtent) {
		...
		for (let j = 0; j < data.length; j++) {
			...
			if (typeof yExtent === "function") {
				...
				} else lo = hi = Utils$1.parseNumber(y);
			}
			start.push(lo);
			end.push(hi);
		}
	...
	*/
	handleCandleStickBoxData(ser, i2) {
		...
		if (this.isFormat2DArray()) ohlc = this.handleCandleStickBoxDataFormat("array", ser, i2);
		else if (this.isFormatXY()) ohlc = this.handleCandleStickBoxDataFormat("xy", ser, i2);
	...
	*/
	handleViolinData(ser, i2) {
		...
		for (let j = 0; j < data.length; j++) {
			...
			const summary = this._parseViolinSummary((_h = (_g = d == null ? void 0 : d.y) == null ? void 0 : _g.summary) != null ? _h : d == null ? void 0 : d[3]);
			const values = [];
			const weights = [];
			let maxWeight = 0;
			let modeValue = null;
		...
		w.violinData.seriesViolinSummary[i2] = summaryArr;
	...
	*/
	handleRangeDataFormat(format, ser, i2) {
		...
		if (format === "array") for (let j = 0; j < ser[i2].data.length; j++) if (Array.isArray(ser[i2].data[j])) {
		...
		else if (format === "xy") for (let j = 0; j < ser[i2].data.length; j++) {
	...
	*/
	handleCandleStickBoxDataFormat(format, ser, i2) {
		...
		if (format === "array") {
			const first = data[0];
			if (isBoxPlot && first && first.length === 6 || !isBoxPlot && first && first.length === 5) getVals = (d) => d.slice(1);
	...
	*/
	parseDataAxisCharts(ser) {
		...
		ser.forEach((s2, i2) => {
			if (s2.name !== void 0) this.w.seriesData.seriesNames.push(s2.name);
			else this.w.seriesData.seriesNames.push("series-" + parseInt(String(i2 + 1), 10));
		});
		this.coreUtils.setSeriesYAxisMappings();
		const buckets = [];
		...
		for (let i2 = 0; i2 < ser.length; i2++) {
			...
			if (isCustom(customType)) {
				const cls = getChartClass(customType);
			...
			if (this.isMultiFormat()) {
				if (this.isFormat2DArray()) this.handleFormat2DArray(ser, i2);
				else if (this.isFormatXY()) this.handleFormatXY(ser, i2);
				if (cnf.chart.type === "candlestick" || ser[i2].type === "candlestick" || cnf.chart.type === "boxPlot" || ser[i2].type === "boxPlot") this.handleCandleStickBoxData(ser, i2);
				if (cnf.chart.type === "violin" || ser[i2].type === "violin") this.handleViolinData(ser, i2);
				...
				if (i2 === this.activeSeriesIndex && !this.fallbackToCategory) this.w.axisFlags.isXNumeric = true;
			} else {
				if (cnf.xaxis.type === "datetime") {
					this.w.axisFlags.isXNumeric = true;
					handleDates();
			...
			if (ser[i2].color !== void 0) this.w.seriesData.seriesColors.push(ser[i2].color);
	...
	*/
	parseDataNonAxisCharts(ser) {
		...
		const hasNewFormat = Array.isArray(ser) && ser.some((s2) => s2 && typeof s2 === "object" && s2.data || s2 && typeof s2 === "object" && s2.parsing);
		if (cnf.chart.type === "unit" && hasNewFormat && !hasOldFormat) return this.parseUnitSeries(ser);
		...
		if (hasOldFormat) {
			...
			for (let i2 = 0; i2 < this.w.seriesData.series.length; i2++) if (this.w.seriesData.seriesNames[i2] === void 0) this.w.seriesData.seriesNames.push("series-" + (i2 + 1));
			return this.w;
		}
		if (Array.isArray(ser) && ser.every((s2) => typeof s2 === "number")) {
			this.w.seriesData.series = ser.slice();
			this.w.seriesData.seriesNames = [];
			for (let i2 = 0; i2 < this.w.seriesData.series.length; i2++) this.w.seriesData.seriesNames.push(cnf.labels[i2] || `series-${i2 + 1}`);
			return this.w;
		}
	...
	*/
	parseUnitSeries(ser) {
		...
		this.w.seriesData.series = series;
	...
	_generateExternalLabels(ser) {
		...
		if (gl.axisCharts) {
			if (this.w.seriesData.series.length > 0) if (this.isFormatXY()) {
				...
				const len = seriesDataFiltered.reduce((p, c2, i2, a2) => a2[p].length > c2.length ? p : i2, 0);
				for (let i2 = 0; i2 < seriesDataFiltered[len].length; i2++) labelArr.push(i2 + 1);
			} else for (let i2 = 0; i2 < this.w.seriesData.series[gl.maxValsInArrayIndex].length; i2++) labelArr.push(i2 + 1);
			this.w.seriesData.seriesX = [];
			for (let i2 = 0; i2 < ser.length; i2++) this.w.seriesData.seriesX.push(labelArr);
...
this.w.seriesData.series.map((gls, glsi) => {
		...
		if (cnf.xaxis.convertedCatToNumeric) this.w.labelData.categoryLabels = labelArr.map((l2) => {
	...
	*/
	parseRawDataIfNeeded(series) {
		...
		const hasSeriesParsing = series.some((s2) => s2.parsing && (s2.parsing.x || s2.parsing.y || s2.parsing.z));
		if (!hasGlobalParsing && !hasSeriesParsing) return series;
		const processedSeries = series.map((serie, index) => {
			...
			if (!effectiveParsing.x || !effectiveParsing.y || Array.isArray(effectiveParsing.y) && effectiveParsing.y.length === 0) {
				...
				if (!effectiveParsing.y || Array.isArray(effectiveParsing.y) && effectiveParsing.y.length === 0) missing.push("y");
				const seriesName = (_d = serie.name) != null ? _d : `series[${index}]`;
				console.warn(`ApexCharts [${this.w.globals.chartID}]: "${seriesName}" has a parseData config but is missing the '${missing.join("', '")}' field specification.`, { parsing: (_e = serie.parsing) != null ? _e : globalParsing });
				return serie;
			}
	...
	*/
	applySeriesTransform(ser) {
		...
		if (name2 !== "waterfall" && this.w.waterfallData.geometry) this.w.waterfallData = {
			values: [],
			cumulative: [],
			kinds: [],
			geometry: null
		};
		if (name2 !== "dumbbell" && this.w.dumbbellData) this.w.dumbbellData = null;
		if (name2 !== "streamgraph" && this.w.streamgraphData) this.w.streamgraphData = null;
		const transform = getSeriesTransform(name2);
		if (transform) return transform(ser, this.w);
		const feature = RAW_SAMPLE_FEATURES[name2];
		if (!Array.isArray(ser) || !feature) return ser;
		if (!this._warnedMissingTransform) {
			this._warnedMissingTransform = true;
			console.warn(`ApexCharts: chart.type '${name2}' requires the ${feature} feature, which is not in this bundle. Bundler: import 'apexcharts/features/${feature}' (or from 'apexcharts/${name2}'). Script tag: add <script src='.../dist/features/${feature}.js'> after apexcharts.js.`);
		}
	...
	*/
	flattenTreemapHierarchy(ser) {
		...
		if (!gl.treemapRawSeries) {
			...
			gl.treemapRawSeries = ser.map((s2) => __spreadProps(__spreadValues({}, s2), { data: Array.isArray(s2 == null ? void 0 : s2.data) ? s2.data.slice() : s2 == null ? void 0 : s2.data }));
		}
		const { roots, leafSeries, maxDepth } = resolveTreemapTree(w, gl.treemapRawSeries);
	...
	*/
	expandScatterJitterData(ser) {
		...
		if (!ser.some((s2) => Array.isArray(s2 == null ? void 0 : s2.data) && s2.data.some((d) => d && !Array.isArray(d) && Array.isArray(d.y)))) {
			if (cnf.xaxis.type !== "datetime") {
				if (Array.isArray(cnf.xaxis.categories) && cnf.xaxis.categories.length) this._applyBandAxis(cnf.xaxis.categories.slice());
				else if (Array.isArray(cnf.xaxis._scatterBandLabels) && cnf.xaxis._scatterBandLabels.length) this._applyBandAxis(cnf.xaxis._scatterBandLabels);
			}
			return ser;
		}
		const bandLabels = [];
	...
	*/
	_applyBandAxis(bandLabels) {
		...
		if (((_a = this.w.interact) == null ? void 0 : _a.zoomed) && typeof xa.min === "number" && typeof xa.max === "number" && isFinite(xa.min) && isFinite(xa.max)) {
			...
			let first = clampBand(Math.round(xa.min + .49));
			let last = clampBand(Math.round(xa.max - .49));
			if (last < first) first = last = clampBand(Math.round((xa.min + xa.max) / 2));
			xa.min = first - 1;
			xa.max = last + 1;
	...
	*/
	parseData(ser) {
		...
		if (((_a = cnf.chart.dataReducer) == null ? void 0 : _a.enabled) && gl.axisCharts && !gl.dataReducerRawSeries) {
			gl.dataReducerRawSeries = ser.map((s2) => ({ data: Array.isArray(s2 == null ? void 0 : s2.data) ? s2.data.slice() : s2 == null ? void 0 : s2.data }));
		...
		if (gl.dataReducerRawSeries && ((_f = cnf.chart.dataReducer) == null ? void 0 : _f.enabled)) ser = ser.map((s2) => __spreadValues({}, s2));
		cnf.series = ser;
		if (gl.dataReducerRawSeries && ((_g = cnf.chart.dataReducer) == null ? void 0 : _g.enabled)) {
			...
			gl.initialSeries = ser.map((s2, i2) => {
				var _a2, _b2, _c2;
				return __spreadProps(__spreadValues({}, s2), { data: (_c2 = (_b2 = (_a2 = stash[i2]) == null ? void 0 : _a2.data) == null ? void 0 : _b2.slice()) != null ? _c2 : s2.data });
			});
		} else if (gl.histogramRawSeries) gl.initialSeries = gl.histogramRawSeries;
		else if (gl.dumbbellRawSeries) gl.initialSeries = gl.dumbbellRawSeries;
		else if (gl.streamgraphRawSeries) gl.initialSeries = gl.streamgraphRawSeries;
		else if (gl.waterfallRawSeries) gl.initialSeries = gl.waterfallRawSeries;
		else if (gl.treemapRawSeries) gl.initialSeries = gl.treemapRawSeries;
		else gl.initialSeries = ser;
		...
		if (gl.axisCharts) {
			Data._defineLazyResult(this.w.seriesData, "stackedSeriesTotals", () => this.coreUtils.getStackedSeriesTotals());
			Data._defineLazyResult(this.w.seriesData, "stackedSeriesTotalsByGroups", () => this.coreUtils.getStackedSeriesTotalsByGroups());
			Data._defineLazyResult(gl, "seriesPercent", () => {
				this.coreUtils.getPercentSeries();
				return gl.seriesPercent;
			});
		} else this.coreUtils.getPercentSeries();
		...
		for (let l2 = 0; l2 < catLabels.length; l2++) if (Array.isArray(catLabels[l2])) {
		...
		return new Promise((resolve, reject) => {
			...
			charts.forEach((ch, chartIndex) => {
				...
				if (!redraw) {
					...
					if (animate && w.config.chart.animations.enabled) ch.series.getPreviousPaths();
				}
				...
				if (options2 && typeof options2 === "object") {
					...
					if (options2.series && (w.globals.collapsedSeriesIndices.length > 0 || w.globals.ancillaryCollapsedSeriesIndices.length > 0)) ch.series.reconcileCollapsedByName();
				}
	...
	*/
	_updateSeries(newSeries, animate, overwriteInitialSeries = false) {
		return new Promise((resolve, reject) => {
			...
			if (animate && w.config.chart.animations.enabled) this.ctx.series.getPreviousPaths();
			const prevSeriesCount = w.config.series.length;
			const prevDataLengths = w.config.series.map((s2) => {
				var _a, _b;
			...
			if (overwriteInitialSeries) {
				if (w.globals.initialConfig) w.globals.initialConfig.series = w.config.series;
				w.globals.initialSeries = w.config.series;
			}
			if (this._canUseFastPath(newSeries, prevSeriesCount, prevDataLengths, w)) return this.ctx.fastUpdate(animate, prevAxisScaleSig).then(() => {
				resolve(this.ctx);
			}).catch(reject);
...
var Utils2 = class {
	...
	*/
	static hoverTarget(e2) {
		if (!e2) return null;
		if (e2.eventPhase && e2.target) e2.apexHoverTarget = e2.target;
	...
	*/
	getNearestValues({ hoverArea, elGrid, clientX, clientY }) {
		...
		if (hoverX < -edgePad || hoverY < 0 || hoverX > hoverWidth + edgePad || hoverY > hoverHeight) {
		...
		let closest = null;
		const seriesXValArr = w.globals.seriesXvalues.map((seriesXVal) => {
		...
		if (w.axisFlags.isXNumeric) {
			...
			const wasCompacted = seriesXValArr.some((arr, i2) => arr.length !== w.globals.seriesXvalues[i2].length);
			if (capturedSeries !== null && wasCompacted) {
				const unfiltered = w.globals.seriesXvalues[capturedSeries];
				let nearest = null;
				let nearestDiff = Infinity;
				for (let i2 = 0; i2 < unfiltered.length; i2++) {
					if (!Utils$1.isNumber(unfiltered[i2])) continue;
					const diff = Math.abs(hoverX - unfiltered[i2]);
					if (diff < nearestDiff) {
						nearestDiff = diff;
						nearest = i2;
					}
	...
	*/
	getFirstActiveXArray(Xarrays) {
		...
		for (let a2 = 0; a2 < firstActiveSeriesIndex.length; a2++) if (firstActiveSeriesIndex[a2] !== -1 && w.globals.collapsedSeriesIndices.indexOf(a2) === -1 && w.globals.ancillaryCollapsedSeriesIndices.indexOf(a2) === -1) {
	...
	*/
	isXoverlap(j) {
		...
		if (seriesX.length > 0) {
			for (let i2 = 0; i2 < seriesX.length - 1; i2++) if (typeof seriesX[i2][j] !== "undefined" && typeof seriesX[i2 + 1][j] !== "undefined") {
				if (seriesX[i2][j] !== seriesX[i2 + 1][j]) xSameForAllSeriesJArr.push("unEqual");
			}
		}
	...
	isInitialSeriesSameLen() {
		var _a, _b, _c, _d;
		let sameLen = true;
		const initialSeries = ((_b = (_a = this.w.globals._initialSeriesPeek) != null ? _a : this.w.globals.initialSeries) == null ? void 0 : _b.filter(
			/**
	...
	*/
	toggleAllTooltipSeriesGroups(state) {
		...
		for (let i2 = 0; i2 < allTooltipSeriesGroups.length; i2++) if (state === "enable") {
...
var Labels = class {
	...
	*/
	...
	drawSeriesTexts({ shared = true, ttItems, i: i2 = 0, j = null, y1, y2, e: e2 }) {
		...
		else this.toggleActiveInactiveSeries(shared, i2);
	...
	*/
	computeSeriesRow({ i: i2, j, tIndex, shared, e: e2, basePColor }) {
		...
		if (j === null) val = f.yLbFormatter(w.seriesData.series[i2], __spreadProps(__spreadValues({}, w), {
			seriesIndex: i2,
			dataPointIndex: i2
		}));
		return {
	...
	*/
	formatYValue(f, index, j) {
		...
		if (steps && steps[index] && steps[index][j] != null) return f.yLbFormatter(steps[index][j], {
			series: steps,
			seriesIndex: index,
			dataPointIndex: j,
			w
	...
	*/
	...
	DOMHandling({ t: t2, j, ttItems, values, seriesName, shared, pColor }) {
		...
		const ttYLabel = ttItems[t2].querySelector(".apexcharts-tooltip-text-y-label");
		...
		if (ttItemsChildren[0] && ttItemsChildren[0].classList.contains("apexcharts-tooltip-marker")) {
			if (w.config.tooltip.marker.fillColors && Array.isArray(w.config.tooltip.marker.fillColors)) pColor = w.config.tooltip.marker.fillColors[t2];
		...
		const ttGLabel = ttItems[t2].querySelector(".apexcharts-tooltip-text-goals-label");
		const ttGVal = ttItems[t2].querySelector(".apexcharts-tooltip-text-goals-value");
		if (goalVals.length && w.seriesData.seriesGoals[t2]) {
			...
			if (shared) if (w.seriesData.seriesGoals[t2][j] && Array.isArray(w.seriesData.seriesGoals[t2][j])) createGoalsHtml();
			else {
				ttGLabel.innerHTML = "";
				ttGVal.innerHTML = "";
			}
		...
		if (zVal !== null) {
			const ttZLabel = ttItems[t2].querySelector(".apexcharts-tooltip-text-z-label");
			ttZLabel.innerHTML = w.config.tooltip.z.title;
			const ttZVal = ttItems[t2].querySelector(".apexcharts-tooltip-text-z-value");
			ttZVal.innerHTML = typeof zVal !== "undefined" ? zVal : "";
		}
		if (shared && ttItemsChildren[0]) {
			if (w.config.tooltip.hideEmptySeries) {
				const ttItemMarker = ttItems[t2].querySelector(".apexcharts-tooltip-marker");
			...
			if (typeof val === "undefined" || val === null || w.globals.ancillaryCollapsedSeriesIndices.indexOf(t2) > -1 || w.globals.collapsedSeriesIndices.indexOf(t2) > -1 || Array.isArray(ttCtx.tConfig.enabledOnSeries) && ttCtx.tConfig.enabledOnSeries.indexOf(t2) === -1) ttItemsChildren[0].parentNode.style.display = "none";
			else ttItemsChildren[0].parentNode.style.display = w.config.tooltip.items.display;
		} else if (Array.isArray(ttCtx.tConfig.enabledOnSeries) && ttCtx.tConfig.enabledOnSeries.indexOf(t2) === -1) ttItemsChildren[0].parentNode.style.display = "none";
	}
	/**
	...
	*/
	toggleActiveInactiveSeries(shared, i2) {
		...
		if (shared) this.tooltipUtil.toggleAllTooltipSeriesGroups("enable");
			...
			const firstTooltipSeriesGroup = w.dom.baseEl.querySelector(`.apexcharts-tooltip-series-group-${i2}`);
	...
	getValuesToPrint({ i: i2, j }) {
		...
		if (j === null) val = w.seriesData.series[i2];
		else if (w.axisFlags.isXNumeric && w.config.chart.type !== "treemap") {
			xVal = filteredSeriesX[i2][j];
			if (filteredSeriesX[i2].length === 0) xVal = filteredSeriesX[this.tooltipUtil.getFirstActiveXArray(filteredSeriesX)][j];
		} else if (new Data(this.w).isFormatXY()) xVal = typeof w.config.series[i2].data[j] !== "undefined" ? w.config.series[i2].data[j].x : "";
		...
		if (w.seriesData.seriesZ.length > 0 && w.seriesData.seriesZ[i2].length > 0) zVal = zFormatter == null ? void 0 : zFormatter(w.seriesData.seriesZ[i2][j], w);
...
var Position = class {
	...
	*/
	computeTooltipPosition(cx, cy, markerSize = null) {
		...
		if (x > w.layout.gridWidth - ttW - 10) x = placement === "left" ? Math.min(w.layout.gridWidth - ttW, x) : w.layout.gridWidth - ttW;
		if (x < -20) x = -20;
	...
	*/
	moveMarkers(i2, j) {
		...
		if (w.globals.markers.size[i2] > 0 && !w.globals.markers.batched) {
			const allPoints = w.dom.baseEl.querySelectorAll(` .apexcharts-series[data\\:realIndex='${i2}'] .apexcharts-marker`);
	...
	*/
	moveStickyTooltipOverBars(j, capturedSeries) {
		...
		if (w.globals.isBarHorizontal) i2 = new Series(this.w).getActiveConfigSeriesIndex("desc") + 1;
		let jBar = w.dom.baseEl.querySelector(`.apexcharts-bar-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-candlestick-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-boxPlot-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-violin-series .apexcharts-series[rel='${i2}'] path[j='${j}'], .apexcharts-rangebar-series .apexcharts-series[rel='${i2}'] path[j='${j}']`);
		...
		if (!jBar && bcc) {
			...
			if (!bc) {
				for (const key in bcc) if ((_b = bcc[key]) == null ? void 0 : _b[j]) {
					bc = bcc[key][j];
					break;
				}
			}
		...
		} else if (!w.globals.isBarHorizontal && !bc) {
...
var Marker = class {
	...
	*/
	...
	drawDynamicPoints() {
		...
		if (w.config.chart.stacked) elsSeries.sort((a2, b) => {
		...
		for (let i2 = 0; i2 < elsSeries.length; i2++) {
			const pointsMain = elsSeries[i2].querySelector(`.apexcharts-series-markers-wrap`);
			if (pointsMain !== null) {
				let PointClasses = `apexcharts-marker w${(Math.random() + 1).toString(36).substring(4)}`;
				if (Markers.markersAreInert(w)) PointClasses += " no-pointer-events";
	...
	*/
	enlargeCurrentPoint(rel, point, x = null, y = null) {
		...
		if (w.config.chart.type !== "bubble") appliedSize = this.newPointSize(rel, point);
...
var Intersect = class {
	...
	*/
	...
	handleHeatTreeTooltip({ e: e2, opt, x, y, type }) {
		...
		if (canvasCells) {
			...
			const hit = renderer.hitTest(clientX - seriesBound.left, clientY - seriesBound.top);
			if (!hit) return {
				x,
				y,
				noHit: true
	...
	*/
	...
	handleMarkerTooltip({ e: e2, opt, x, y }) {
		...
		if (Utils2.hoverTarget(e2).classList.contains("apexcharts-marker")) {
			...
			i2 = parseInt(opt.paths.parentNode.parentNode.parentNode.getAttribute("rel"), 10) - 1;
			if (ttCtx.intersect) {
				const el = Utils$1.findAncestor(opt.paths, "apexcharts-series");
				if (el) i2 = parseInt(el.getAttribute("data:realIndex"), 10);
			}
			ttCtx.tooltipLabels.drawSeriesTexts({
				ttItems: opt.ttItems,
				i: i2,
			...
			if (e2.type === "mouseup") ttCtx.markerClick(e2, i2, j);
			w.interact.capturedSeriesIndex = i2;
			w.interact.capturedDataPointIndex = j;
			const arrowEnabled = !!w.config.tooltip.arrow;
			x = cx;
	...
	*/
	...
	handleBarTooltip({ e: e2, opt }) {
		...
		if (ttCtx.tooltip === null) ttCtx.tooltip = w.dom.baseEl.querySelector(".apexcharts-tooltip:not(.apexcharts-annotation-tooltip)");
	...
	getBarTooltipXY({ e: e2, opt }) {
		...
		if (cl.contains("apexcharts-bar-area") || cl.contains("apexcharts-candlestick-area") || cl.contains("apexcharts-boxPlot-area") || cl.contains("apexcharts-rangebar-area")) {
			...
			if (w.globals.comboCharts) i2 = parseInt(bar.parentNode.getAttribute("data:realIndex"), 10);
	...
	**/
	drawYaxisTooltip() {
		...
		for (let i2 = 0; i2 < w.config.yaxis.length; i2++) {
			...
			const renderTo = w.dom.elWrap;
			if (w.dom.baseEl.querySelector(`.apexcharts-yaxistooltip.apexcharts-yaxistooltip-${i2}`) === null) {
...
var Tooltip = class {
	...
	*/
	getElTooltip(ctx) {
		...
		return ctx.w.dom.baseEl.querySelector(".apexcharts-tooltip:not(.apexcharts-annotation-tooltip)");
	}
	getElXCrosshairs() {
	...
	*/
	drawTooltip(xyRatios) {
		...
		if (this.tConfig.compact) {
			...
			if (w.config.series.length === 1) tooltipEl.classList.add("apexcharts-tooltip-value-only");
		}
		...
		if (w.config.markers.size === 0 || w.globals.markers.largestSize === 0 || w.globals.markers.batched) this.marker.drawDynamicPoints();
	...
	*/
	createTTElements(ttItemsCnt) {
		...
		for (let i2 = 0; i2 < ttItemsCnt; i2++) {
			...
			gTxt.classList.add("apexcharts-tooltip-series-group", `apexcharts-tooltip-series-group-${i2}`);
			gTxt.style.order = String(w.config.tooltip.inverseOrder ? ttItemsCnt - i2 : i2 + 1);
			const point = BrowserAPIs.createElementNS("http://www.w3.org/1999/xhtml", "span");
			point.classList.add("apexcharts-tooltip-marker");
			if (w.config.tooltip.fillSeriesColor) point.style.backgroundColor = w.globals.colors[i2];
			else point.style.color = w.globals.colors[i2];
			const mShape = w.config.markers.shape;
			let shape = mShape;
			if (Array.isArray(mShape)) shape = mShape[i2];
	...
	addSVGEvents() {
		...
		else if (commonBar && !w.globals.comboCharts || chartWithmarkers && this.showOnIntersect || isPolarMarkerChart) this.addDatapointEventsListeners(seriesHoverParams);
		else if (type === "heatmap" && w.globals.activeRenderer && w.globals.activeRenderer.kind === "canvas") this.addPathsEventListeners([hoverArea], seriesHoverParams);
	...
	*/
	...
	onSeriesHover(opt, e2) {
		...
		if (timeSinceLastUpdate >= targetDelay) this.seriesHover(opt, e2);
		else {
	...
	seriesHover(opt, e2) {
		...
		if (w.config.chart.group && !isCfMember(this.ctx)) chartGroups = this.ctx.getSyncedCharts().filter((ch) => !isCfMember(ch));
	...
	seriesHoverByContext({ chartCtx, ttCtx, opt, e: e2 }) {
		...
		if (ttCtx.tooltipUtil.hasBars() && !w.globals.comboCharts && !ttCtx.isBarShared) {
			if (this.tConfig.onDatasetHover.highlightDataSeries) new Series(chartCtx.w).toggleSeriesOnHover(e2, (_a = Utils2.hoverTarget(e2)) == null ? void 0 : _a.parentNode);
		}
	...
	axisChartsTooltips({ e: e2, opt }) {
		...
		if (w.dom.elWrap.querySelector(".apexcharts-annotation-tooltip.apexcharts-active")) {
			this.handleMouseOut(opt);
			return;
		}
		...
		let syncedCharts = [];
		if (w.config.chart.group && !isCellChart) syncedCharts = this.ctx.getSyncedCharts();
		...
		if (e2.type === "mousemove" || e2.type === "touchmove" || e2.type === "mouseup") {
			...
			if (!isCellChart && (isStickyTooltip && !this.showOnIntersect || syncedCharts.length > 1)) this.handleStickyTooltip(e2, clientX, clientY, opt);
		...
		} else if (e2.type === "mouseout" || e2.type === "touchend") this.handleMouseOut(opt);
	}
	/**
	...
	*/
	...
	nonAxisChartsTooltips({ e: e2, opt, tooltipRect }) {
		...
		if (e2.type === "mousemove" || e2.type === "touchmove") {
			...
			} else this.tooltipLabels.drawSeriesTexts({
				ttItems: opt.ttItems,
				i: parseInt(rel, 10) - 1,
				shared: false
			});
			...
			if (w.config.legend.tooltipHoverFormatter) {
				...
				const i2 = rel - 1;
				const legendEl = (_c = this.legendLabels) == null ? void 0 : _c[i2];
				if (!legendEl) return;
			...
			if (w.config.legend.tooltipHoverFormatter) (_d = this.legendLabels) == null || _d.forEach((l2) => {
				const defaultText = l2.getAttribute("data:default-text");
				l2.innerHTML = decodeURIComponent(defaultText != null ? defaultText : "");
			});
		}
	}
	/**
	...
	*/
	renderUnitTooltip(dotEl) {
		...
		if (typeof w.config.tooltip.custom === "function") {
			this.tooltipLabels.handleCustomTooltip({
				i: i2,
				j,
				y1: null,
				y2: null,
		...
		if (typeof fmt === "function") body = fmt({
			seriesName,
			seriesIndex: i2,
			dataPointIndex: j,
			count,
			...
			if (unitValue !== 1) body += ` &middot; ${unitValue.toLocaleString()} per dot`;
		}
	...
	*/
	handleStickyTooltip(e2, clientX, clientY, opt) {
		...
		if (capturedSeries !== null) this.handleStickyCapturedSeries(e2, capturedSeries != null ? capturedSeries : -1, opt, j != null ? j : 0);
	...
	*/
	handleStickyCapturedSeries(e2, capturedSeries, opt, j) {
		...
		if (typeof w.seriesData.series[capturedSeries][j] !== "undefined") if (this.tConfig.shared && this.tooltipUtil.isXoverlap(j) && this.tooltipUtil.isInitialSeriesSameLen()) this.create(e2, this, capturedSeries, j, opt.ttItems);
		else this.create(e2, this, capturedSeries, j, opt.ttItems, false);
	...
	*/
	handleMouseOut(opt) {
		...
		if (this.yaxisTooltips && this.yaxisTooltips.length) {
			...
			for (let i2 = 0; i2 < this.yaxisTTEls.length; i2++) this.yaxisTTEls[i2].classList.remove("apexcharts-active");
		}
		if (w.config.legend.tooltipHoverFormatter) (_b = this.legendLabels) == null || _b.forEach((l2) => {
			const defaultText = l2.getAttribute("data:default-text");
			l2.innerHTML = decodeURIComponent(defaultText != null ? defaultText : "");
		});
	}
	...
	*/
	markerClick(e2, seriesIndex, dataPointIndex) {
		const w = this.w;
		if (typeof w.config.chart.events.markerClick === "function") w.config.chart.events.markerClick(e2, this.ctx, {
			seriesIndex,
			dataPointIndex,
			w
		});
	...
	*/
	_hasCustomSeries() {
		...
		return (w.config.series || []).some((s2) => s2 && s2.type && isCustom(s2.type));
	}
	/**
	* @param {Event} e
	* @param {any} context
	...
	*/
	create(e2, context, capturedSeries, j, ttItems, shared = null) {
		...
		const w = this.w;
		const ttCtx = context;
		if (e2.type === "mouseup") this.markerClick(e2, capturedSeries, j);
		...
		const handlePoints = () => {
			if (w.globals.markers.largestSize > 0 && !canvasMode && !w.globals.markers.batched) ttCtx.marker.enlargePoints(j);
			else ttCtx.tooltipPosition.moveDynamicPointsOnHover(j);
		};
		if (w.config.legend.tooltipHoverFormatter) {
			const legendFormatter = w.config.legend.tooltipHoverFormatter;
			const els = Array.from((_c = this.legendLabels) != null ? _c : []);
			els.forEach((l2) => {
			...
			for (let i2 = 0; i2 < els.length; i2++) {
				...
				const legendName = decodeURIComponent((_e = l2.getAttribute("data:default-text")) != null ? _e : "");
				...
				} else l2.innerHTML = w.globals.collapsedSeriesIndices.indexOf(lsIndex) < 0 ? text : legendName;
			}
		}
		...
		const commonSeriesTextsParams = __spreadValues(__spreadValues({
			...
			i: capturedSeries,
			j
		}, ((_i = (_h = (_g = (_f = _rangeData.seriesRange) == null ? void 0 : _f[capturedSeries]) == null ? void 0 : _g[j]) == null ? void 0 : _h.y[0]) == null ? void 0 : _i.y1) !== void 0 && { y1: (_m = (_l = (_k = (_j = _rangeData.seriesRange) == null ? void 0 : _j[capturedSeries]) == null ? void 0 : _k[j]) == null ? void 0 : _l.y[0]) == null ? void 0 : _m.y1 }), ((_q = (_p = (_o = (_n = _rangeData.seriesRange) == null ? void 0 : _n[capturedSeries]) == null ? void 0 : _o[j]) == null ? void 0 : _p.y[0]) == null ? void 0 : _q.y2) !== void 0 && { y2: (_u = (_t = (_s = (_r = _rangeData.seriesRange) == null ? void 0 : _r[capturedSeries]) == null ? void 0 : _s[j]) == null ? void 0 : _t.y[0]) == null ? void 0 : _u.y2 });
		if (shared) {
			...
			if (hasMarkers || dynamicPoints) handlePoints();
			else if (this.tooltipUtil.hasBars()) {
				if (canvasMode) ttCtx.tooltipPosition.moveStickyTooltipOverBars(j, capturedSeries);
			...
			else if (dynamicPoints) ttCtx.tooltipPosition.moveDynamicPointOnHover(j, capturedSeries);
		}
	}
};
...
var RENDERER_REGISTRY_KEY = "__apexcharts_renderers__";
...
var RendererController = class {
	...
	*/
	_desiredKind() {
		...
		return computeMarkCount(this.w) >= (cfg.rendererThreshold || 8e3) ? "canvas" : "svg";
	}
	/**
	...
	*/
	resolve() {
		...
		if (desired !== "svg") {
			...
			if (factory) {
				if (!this._instances[desired]) this._instances[desired] = factory(this.w, this.ctx);
			...
			if (mode === desired) console.warn(`[apexcharts] renderer:"${desired}" requested but that renderer is not in the default bundle. Bundler: import 'apexcharts/features/renderer-${desired}'. Script tag: add <script src=".../dist/features/renderer-${desired}.js"> after apexcharts.js. Falling back to SVG.`);
		} else if (mode === "canvas" && hasCanvasUnsupportedFeature(this.w)) console.warn(`[apexcharts] renderer:"canvas" requested but this chart uses a feature the canvas renderer does not render yet (gradient/pattern/image fill or a state color-matrix filter); falling back to SVG.`);
		this.active = this.svg;
		this._activeKind = "svg";
		this.ctx.renderer = this.active;
		this.w.globals.activeRenderer = this.active;
...
var SVGElement = class SVGElement {
	...
	*/
	attr(a2, v) {
		if (typeof a2 === "string" && v === void 0) return this.node.getAttribute(a2);
		...
		if (this.node.nodeName === "text" && attrs.x != null) {
			...
			for (let i2 = 0; i2 < tspans.length; i2++) tspans[i2].setAttribute("x", attrs.x);
		}
		return this;
	}
	/**
	...
	*/
	find(selector) {
		return Array.from(this.node.querySelectorAll(selector)).map((n2) => n2.instance || new SVGElement(n2));
	}
	/**
	* @param {string} selector
	*/
...
function installDraggable(ElementClass) {
	ElementClass.prototype.draggable = function(opts) {
		...
		const onPointerDown = (e2) => {
			...
			e2.stopPropagation();
			const ev = e2.type === "touchstart" ? e2.touches[0] : e2;
			const svgEl = el.node;
			...
			const onMove = (me) => {
				...
				if (constraints.maxY !== void 0 && newY + h2 > constraints.maxY) newY = constraints.maxY - h2;
				const box = {
					x: newX,
					y: newY,
					w,
	...
	ElementClass.prototype.resize = function(enable) {
		...
		const makeHandleDraggable = (name2) => {
			...
			const onPointerDown = (e2) => {
				if (e2.button && e2.button !== 0) return;
				e2.stopPropagation();
				const startClientX = (e2.type === "touchstart" ? e2.touches[0] : e2).clientX;
...
function SVG() {
	const svg = new SVGContainer(BrowserAPIs.createElementNS(SVGNS$1, "svg"));
...
var FEATURE_REGISTRY_KEY = "__apexcharts_features_v1__";
if (!globalThis[FEATURE_REGISTRY_KEY]) globalThis[FEATURE_REGISTRY_KEY] = /* @__PURE__ */ new Map();
function getFeatureRegistry() {
	return globalThis[FEATURE_REGISTRY_KEY];
}
var InitCtxVariables = class InitCtxVariables {
	/**
	...
	*/
	static registerFeatures(featureMap) {
		for (const [key, Ctor] of Object.entries(featureMap)) InitCtxVariables._featureRegistry.set(key, Ctor);
	}
	/**
	* @param {import('../../types/internal').ChartContext} ctx
	*/
...
var Destroy = class {
	...
	*/
	clear({ isUpdating }) {
		...
		(_a = this.ctx.weave) == null || _a.teardown(isUpdating);
		...
		if (isUpdating) {
			...
			(_b = this.ctx._keyboardNavigation) == null || _b.destroy();
			this.ctx._keyboardNavigation = null;
		} else {
			(_c = this.ctx.perspectives) == null || _c.teardown();
			this.ctx.perspectives = null;
			(_d = this.ctx.storyboard) == null || _d.teardown();
			this.ctx.storyboard = null;
			(_e = this.ctx.history) == null || _e.teardown();
			this.ctx.history = null;
			(_f = this.ctx.linkedViews) == null || _f.teardown();
			this.ctx.linkedViews = null;
			(_g = this.ctx.trellis) == null || _g.teardown();
			this.ctx.trellis = null;
			(_h = this.ctx.ink) == null || _h.teardown();
			this.ctx.ink = null;
			(_i = this.ctx.measure) == null || _i.teardown();
			this.ctx.measure = null;
			(_j = this.ctx.contextMenu) == null || _j.teardown();
			this.ctx.contextMenu = null;
			(_k = this.ctx.osThemeWatcher) == null || _k.teardown();
			this.ctx.osThemeWatcher = null;
			this.ctx.weave = null;
			(_m = (_l = this.ctx.rendererController) == null ? void 0 : _l.teardown) == null || _m.call(_l);
...
function registerUnitLayout(name2, fn) {
	...
	if (typeof fn !== "function") {
		console.warn(`ApexCharts: registerUnitLayout("${name2}") expects a function (objects, rect) => [{id, x, y}].`);
		return;
	}
	getLayouts()[name2] = fn;
}
...
function normalizeUnitMark(def, name2) {
	...
	if (!vb.every((n2) => isFinite(n2)) || vb[2] <= 0 || vb[3] <= 0) return null;
...
function registerUnitMark(name2, def) {
	...
	if (!mark) {
		console.warn(`ApexCharts: registerUnitMark("${name2}") expects path data, or {path, viewBox?, fillRule?}.`);
		return;
	}
	getMarks()[name2] = mark;
}
...
function registerRowSource(name2, fn) {
	...
	if (typeof fn !== "function") {
		console.warn(`ApexCharts: registerRowSource("${name2}") expects a function (w, opts) => series.`);
		return;
	}
	getSources()[name2] = fn;
}
...
function registerPlugin(def) {
	if (!def || typeof def.name !== "string" || typeof def.setup !== "function") {
		console.error("[apexcharts] registerPlugin: a plugin needs a { name, setup } shape.");
		return;
	}
...
function addResizeListener(el, fn) {
	...
	const ro = new ResizeObserver((r2) => {
		if (called) fn.call(el, r2);
		called = true;
	});
	if (el.nodeType === Node.DOCUMENT_FRAGMENT_NODE) Array.from(el.children).forEach((c2) => ro.observe(c2));
	else ro.observe(el);
	ros.set(fn, ro);
}
...
var i = "APEX-";
var n = /* @__PURE__ */ new Date("2027-07-31T00:00:00Z");
var r = "__apex_license_v1__";
function a() {
	const e2 = globalThis;
	let t2 = e2[r];
	return t2 || (t2 = {
...
var l = class {
	...
	static getLicenseStatus() {
		return this.licenseKey ? (this.validationResult = this.validateKey(this.licenseKey), this.validationResult) : {
			expired: false,
	...
	static setLicense(e2) {
		var _a;
		var i2;
		if (!e2) return this.licenseKey = null, void this.publish({
			expired: false,
			signatureVerified: false,
			valid: false
		});
	...
	static validateKey(e2) {
		...
		return false === i2 ? {
			...
			valid: false
		} : (void 0 === i2 && this.verifySignature(e2, t2, s2), __spreadProps(__spreadValues({}, s2), { signatureVerified: true === i2 }));
	}
	...
	static parseKey(e2) {
		...
		try {
			...
			return s2.issueDate && s2.expiryDate && s2.plan ? {
				data: {
					domains: Array.isArray(s2.domains) ? s2.domains : void 0,
					expiryDate: s2.expiryDate,
					issueDate: s2.issueDate,
					plan: s2.plan,
					valid: true
				},
				signature: "string" == typeof s2.sig && s2.sig ? s2.sig : null
			} : null;
		} catch (e3) {
			return null;
		}
	...
	static validateStructure(e2, t2) {
		...
		if (new Date(r2.expiryDate) < /* @__PURE__ */ new Date()) return {
			data: r2,
			expired: true,
			message: `License expired on ${r2.expiryDate}. Please renew your license.`,
			signatureVerified: false,
			valid: false
		};
		if (r2.domains && r2.domains.length > 0) {
			const e3 = "undefined" == typeof location ? "" : location.hostname;
			if (!r2.domains.some(((t3) => e3 === t3 || e3.endsWith(`.${t3}`)))) return {
				data: r2,
				expired: false,
				message: `License is not valid for this domain (${e3}). Allowed domains: ${r2.domains.join(", ")}.`,
				signatureVerified: false,
				valid: false
			};
		}
	...
	static verifySignature(e2, i2, n2) {
		return __async(this, null, function* () {
			...
			for (const e3 of this.publicKeysSpki) {
				try {
					const t2 = yield l2.importKey("spki", this.base64ToBytes(e3), {
						name: "ECDSA",
						namedCurve: "P-256"
					}, false, ["verify"]);
					c2 = yield l2.verify({
						hash: "SHA-256",
						name: "ECDSA"
					}, t2, this.base64ToBytes(i2.signature), o2);
				} catch (e4) {
					c2 = false;
				}
				if (c2) break;
			...
			if (this.verdicts.set(e2, c2), c2) {
				...
				this.licenseKey === e2 ? this.publish(t2) : this.notify(t2);
				return;
			}
			const h2 = "Invalid license key. The license signature does not verify.", d = {
				data: i2.data,
				expired: false,
				message: h2,
				signatureVerified: true,
...
var c = class {
	static applyStyles(e2) {
		Object.assign(e2.style, this.CRITICAL_STYLES, {
...
c.WATERMARK_ATTR = "data-apexcharts-watermark", c.WATERMARK_TEXT = "APEXCHARTS", c.ATTR = "data-apexcharts-watermark", c.CRITICAL_STYLES = {
	bottom: "0",
	display: "block",
	left: "0",
	msUserSelect: "none",
	...
	zIndex: "10000"
}, c.managed = /* @__PURE__ */ new Set(), c.subscribed = false;
var h = c;
var PRICING_URL = "https://apexcharts.com/pricing";
var _perspectivesTokenDecoded = false;
var enforced = /* @__PURE__ */ new Set();
...
function premiumFeaturesInUse(w, ctx) {
	...
	if (ctx.trellis && typeof ctx.trellis.isActive === "function" && ctx.trellis.isActive()) used.push("trellis");
	if (ctx.storyboard && ctx.storyboard._used) used.push("storyboard");
	const link = chart.link;
	if (ctx.linkedViews && link && (link.enabled === true || typeof link.dimension === "function")) used.push("link");
	if (ctx.ink && chart.ink && chart.ink.enabled === true) used.push("ink");
	if (ctx.measure && chart.measure && chart.measure.enabled === true) used.push("measure");
	if (ctx.contextMenu && chart.contextMenu && chart.contextMenu.enabled === true) used.push("context-menu");
	if (ctx.perspectives && (ctx.perspectives._used || _perspectivesTokenDecoded)) used.push("perspectives");
	if (ctx.history && chart.history && chart.history.enabled === true) used.push("history");
	return used;
}
...
var PREMIUM_PLANS = /* @__PURE__ */ new Set(["premium", "enterprise"]);
...
function notifyTrial(ctx, key, features) {
	...
	if (!key) {
		console.warn(`[ApexCharts] Premium feature${many ? "s" : ""} in use (${features.join(", ")}) without a license. Running in trial mode with a watermark. Get a license: ${PRICING_URL}`);
		return;
	}
	const result = o.validateKey(key);
	if (result.valid) {
		const plan = result.data && result.data.plan || "current";
		console.warn(`[ApexCharts] Premium feature${many ? "s" : ""} in use (${features.join(", ")}) require a Premium or Enterprise license; the ${plan} plan does not include ${many ? "them" : "it"}. Running in trial mode with a watermark. Upgrade: ${PRICING_URL}`);
		return;
	}
	if (key !== o.getKey()) console.error(`[Apex] ${result.message}`);
}
...
function reevaluateLicenseAcrossCharts() {
	...
	if (Array.isArray(instances)) instances.forEach((entry) => {
...
var _ApexCharts = class _ApexCharts {
	/**
	...
	*/
	constructor(el, opts) {
		...
		/** @type {number | null} */
		__publicField(this, "_parentResizeWaiter", null);
		/** @type {{width: any, viewBoxAdded?: boolean, minHeight?: string} | null} */
		__publicField(this, "_printRestore", null);
		/** @type {any} */
		__publicField(this, "beforePrintHandler");
		/** @type {any} */
		__publicField(this, "afterPrintHandler");
		/** @type {any} */
		...
		/** @type {any} */
		__publicField(this, "waterfall");
		/** @type {any} */
		__publicField(this, "streamgraph");
		/** @type {any} */
	...
	render() {
		...
		const renderPromise = new Promise((resolve, reject) => {
			...
			if (Utils$1.elementExists(this.el)) {
				...
				if (wantsTrellis && !this.trellis) console.warn("ApexCharts: `trellis` requires the trellis feature, which is not in the default bundle. Bundler: import 'apexcharts/features/trellis'. Script tag: add <script src='.../dist/features/trellis.js'> after apexcharts.js. Rendering as a single chart.");
				if (((_b2 = (_a2 = this.w.config.chart) == null ? void 0 : _a2.measure) == null ? void 0 : _b2.enabled) && !this.measure) console.warn("ApexCharts: `chart.measure` requires the measure feature, which is not in the default bundle. Bundler: import 'apexcharts/features/measure'. Script tag: add <script src='.../dist/features/measure.js'> after apexcharts.js.");
				if (((_d = (_c = this.w.config.chart) == null ? void 0 : _c.link) == null ? void 0 : _d.enabled) && !this.linkedViews) console.warn("ApexCharts: `chart.link` requires the link feature, which is not in the default bundle. Bundler: import 'apexcharts/features/link'. Script tag: add <script src='.../dist/features/link.js'> after apexcharts.js.");
				if (!this.ink) {
					...
					if (inkOn || anyDraggable) console.warn("ApexCharts: `chart.ink` / `annotations.points[].draggable` requires the ink feature, which is not in the default bundle. Bundler: import 'apexcharts/features/ink'. Script tag: add <script src='.../dist/features/ink.js'> after apexcharts.js.");
				}
				if (((_j = (_i = this.w.config.chart) == null ? void 0 : _i.contextMenu) == null ? void 0 : _j.enabled) && !this.contextMenu) console.warn("ApexCharts: `chart.contextMenu` requires the context-menu feature, which is not in the default bundle. Bundler: import 'apexcharts/features/context-menu'. Script tag: add <script src='.../dist/features/context-menu.js'> after apexcharts.js.");
				if (((_l = (_k = this.w.config.chart) == null ? void 0 : _k.history) == null ? void 0 : _l.enabled) && !this.history) console.warn("ApexCharts: `chart.history` requires the history feature, which is not in the default bundle. Bundler: import 'apexcharts/features/history'. Script tag: add <script src='.../dist/features/history.js'> after apexcharts.js.");
				if (Environment.isBrowser()) {
					if (!isTrellisHost) {
	...
	*/
	create(ser, opts) {
		...
		ser.forEach((s2, realIndex) => {
			if (s2.hidden) series = this.legend.legendHelpers.getSeriesAfterCollapsing({ realIndex });
		});
		...
		(_a = this.rendererController) == null || _a.resolve();
		(_b = this.weave) == null || _b.dispatch("afterParse");
		this.theme.init();
		new Markers(this.w, this).setGlobalMarkerSize();
		this.formatters.setLabelFormatters();
		this.titleSubtitle.draw();
		if (!gl.noData || gl.collapsedSeries.length === w.seriesData.series.length || w.config.legend.showForSingleSeries) (_c = this.legend) == null || _c.init();
		...
		(_d = this.weave) == null || _d.dispatch("afterScales", { xyRatios });
		...
		(_f = (_e = this.legend) == null ? void 0 : _e.heatmapGradientLegend) == null || _f.repositionToPlot();
	...
	*/
	mount(graphData = null) {
		...
		return new Promise((resolve, reject) => {
			...
			else if (w.globals.allSeriesCollapsed) me.series.handleNoData();
			...
			(_e = me.waterfall) == null || _e.drawConnectors();
			(_f = me.streamgraph) == null || _f.draw();
			...
			(_g = me.annotations) == null || _g.drawAxesAnnotations();
			if (!w.globals.noData) {
				if (Environment.isBrowser() && w.config.tooltip.enabled && !w.globals.noData) (_h = me.w.globals.tooltip) == null || _h.drawTooltip(graphData.xyRatios);
				if (w.config.chart.accessibility.enabled && w.config.chart.accessibility.keyboard.enabled && w.config.chart.accessibility.keyboard.navigation.enabled) (_i = me.keyboardNavigation) == null || _i.init();
				if (Environment.isBrowser() && w.globals.axisCharts && (w.axisFlags.isXNumeric || w.config.xaxis.convertedCatToNumeric || w.axisFlags.isRangeBar)) {
					if (w.config.chart.zoom.enabled || w.config.chart.selection && w.config.chart.selection.enabled || w.config.chart.pan && w.config.chart.pan.enabled) (_j = me.zoomPanSelection) == null || _j.init({ xyRatios: graphData.xyRatios });
				} else {
				...
				if (w.config.chart.toolbar.show && !w.globals.allSeriesCollapsed) (_k = me.toolbar) == null || _k.createToolbar();
			}
			(_l = me.weave) == null || _l.dispatch("draw", {
				pass: "full",
	...
	*/
	destroy() {
		var _a, _b;
		if (this.trellis) this.trellis.teardown();
		...
		if (Environment.isBrowser()) {
			...
			clearTimeout((_b = this._parentResizeWaiter) != null ? _b : void 0);
			this._parentResizeWaiter = null;
			window.removeEventListener("beforeprint", this.beforePrintHandler);
			window.removeEventListener("afterprint", this.afterPrintHandler);
		}
		const chartID = this.w.config.chart.id;
		if (chartID && Array.isArray(Apex._chartInstances)) Apex._chartInstances.forEach((c2, i2) => {
			if (c2.id === Utils$1.escapeString(chartID)) Apex._chartInstances.splice(i2, 1);
		});
	...
	*/
	updateOptions(options2, redraw = false, animate = true, updateSyncedCharts = true, overwriteInitialConfig = true) {
		...
		if (options2 && "series" in options2 && !Array.isArray(options2.series)) {
			console.warn("ApexCharts: updateOptions() ignored `series` because it is not an array.");
		...
		if (options2.series) {
			...
			if (options2.series.length && options2.series[0].data) options2.series = options2.series.map((s2, i2) => {
	...
	*/
	updateSeries(newSeries = [], animate = true, overwriteInitialSeries = true) {
		if (!Array.isArray(newSeries)) {
			console.warn("ApexCharts: updateSeries() ignored the call because the series is not an array.");
			return Promise.resolve(this);
		}
		if (this.trellis && this.trellis._mounted) return this.trellis.updateSeries(newSeries, animate);
		this.data.resetParsingFlags();
	...
	*/
	appendData(newData, overwriteInitialSeries = true) {
		...
		if (me.w.config.chart.animations.enabled) me.series.getPreviousPaths();
		const derivedRaw = me.w.globals.histogramRawSeries || me.w.globals.waterfallRawSeries || me.w.globals.dumbbellRawSeries || me.w.globals.streamgraphRawSeries;
		if (derivedRaw) {
			for (let i2 = 0; i2 < derivedRaw.length; i2++) {
				const src = newData[i2];
				if (src && Array.isArray(src.data) && Array.isArray(derivedRaw[i2].data)) for (let j = 0; j < src.data.length; j++) derivedRaw[i2].data.push(src.data[j]);
			}
			return this.update();
		}
		const newSeries = me.w.config.series.slice();
		for (let i2 = 0; i2 < newSeries.length; i2++) if (newData[i2] !== null && typeof newData[i2] !== "undefined") {
			const srcSerie = newData[i2];
			const dstSerie = newSeries[i2];
			for (let j = 0; j < srcSerie.data.length; j++) dstSerie.data.push(srcSerie.data[j]);
		}
		trimStreamingSeries(newSeries, me.w);
		me.w.config.series = newSeries;
		if (overwriteInitialSeries) me.w.globals.initialSeries = me.w.config.series;
		return this.update();
	}
	/**
	...
	*/
	update(options2) {
		return new Promise((resolve, reject) => {
			if (options2 && this.lastUpdateOptions && !_ApexCharts._optionsTooBigToCompare(options2) && Utils$1.stringifyForCompare(this.lastUpdateOptions) === Utils$1.stringifyForCompare(options2)) return resolve(this);
	...
	_fastAxisChromeRefresh(_xyRatios) {
		...
		try {
			...
			for (let index = 0; index < w.config.yaxis.length; index++) {
				...
				const oldY = w.dom.baseEl.querySelector(`.apexcharts-yaxis[rel='${index}']`);
				if (!oldY) {
			...
			if (elgrid !== null) {
				...
				yAxis.setYAxisTextAlignments();
				w.config.yaxis.map((yaxe, index) => {
					if (gl.ignoreYAxisIndexes.indexOf(index) === -1) yAxis.yAxisTitleRotate(index, yaxe.opposite);
				});
			}
			return true;
		} catch (e2) {
			this._fastAxisBailReason = "error: " + (e2 && e2.message);
			return false;
		}
	}
	/**
	...
	fastUpdate(animate, prevAxisScaleSig) {
		return new Promise((resolve, reject) => {
			...
			try {
				...
				if (scaleChanged) this._updateStats.fastWithAxes++;
				else this._updateStats.fast++;
				(_a = this.weave) == null || _a.dispatch("afterScales", {
					pass: "fast",
					xyRatios
				});
				const rr = this.ctx.renderer;
				...
				const innerEl = w.dom.elGraphical.node;
				innerEl.querySelectorAll((reuseCanvasHost ? "" : ".apexcharts-canvas-series-wrap, ") + ".apexcharts-plot-series, .apexcharts-series, .apexcharts-datalabels, .apexcharts-datalabels-background").forEach((el) => {
					var _a2;
				...
				dataLabels.bringForward();
				if (w.config.dataLabels.background.enabled) dataLabels.dataLabelsBackground();
				(_b = this.waterfall) == null || _b.drawConnectors();
				(_c = this.streamgraph) == null || _c.draw();
				if (!gl.streamScrolled) applyAxisTransition(w);
				applyDataLabelTransition(w);
				if (Environment.isBrowser() && w.config.tooltip.enabled && !gl.noData) (_d = w.globals.tooltip) == null || _d.drawTooltip(xyRatios);
				(_e = this.weave) == null || _e.dispatch("draw", {
					pass: "fast",
					xyRatios
				});
	...
	*/
	getGroupedCharts() {
		return Apex._chartInstances.filter((ch) => this !== ch.chart && !!this.w.config.chart.group && this.w.config.chart.group === ch.group).map((ch) => ch.chart);
	}
	/**
	...
	*/
	static trellis(el, options2) {
		if (!InitCtxVariables._featureRegistry.get("trellis")) {
			console.warn("ApexCharts.trellis requires the trellis feature, which is not in the default bundle. Bundler: import 'apexcharts/features/trellis'. Script tag: add <script src='.../dist/features/trellis.js'> after apexcharts.js.");
			return null;
		}
	...
	*/
	static initOnLoad() {
		...
		for (let i2 = 0; i2 < els.length; i2++) {
			...
			new _ApexCharts(el, options2).render();
		}
	}
	/**
	...
	*/
	static registerSeriesType(name2, def) {
		const factory = _ApexCharts._customSeriesFactory;
		if (!factory) {
			console.warn(`[apexcharts] registerSeriesType("${name2}") requires the Marks feature: import 'apexcharts/features/marks'.`);
			return _ApexCharts;
		}
		if (!def || typeof def.renderItem !== "function") {
			console.warn(`[apexcharts] registerSeriesType("${name2}") needs a def with a renderItem() function.`);
			return _ApexCharts;
		}
		if (hasChartClass(name2) && !isCustom(name2) || TYPE_ALIASES[name2]) {
			console.warn(`[apexcharts] registerSeriesType("${name2}") would override the built-in "${name2}" chart type; pick another name.`);
			return _ApexCharts;
		}
	...
	*/
	static crossfilter(opts) {
		if (!opts || typeof opts.id !== "string") throw new Error("ApexCharts.crossfilter requires an { id } string.");
		const factory = _ApexCharts._crossfilterFactory;
		if (!factory) {
			console.warn(`[apexcharts] ApexCharts.crossfilter(...) requires the link feature, which is not in the default bundle. Bundler: import 'apexcharts/features/link'. Script tag: add <script src='.../dist/features/link.js'> after apexcharts.js.`);
			return null;
		}
	...
	*/
	static getCrossfilter(id) {
		const get = _ApexCharts._crossfilterGet;
	...
	*/
	clearCrossfilter() {
		var _a;
		(_a = this.linkedViews) == null || _a.clearGroup();
	}
	/**
	...
	*/
	startMeasure() {
		var _a;
		(_a = this.measure) == null || _a.startMeasure();
	}
	/** Measure ruler (#18): leave measure mode. */
	stopMeasure() {
		var _a;
		(_a = this.measure) == null || _a.stopMeasure();
	}
	/** Measure ruler (#18): remove all pinned measure rulers. */
	clearMeasures() {
		var _a;
		(_a = this.measure) == null || _a.clearMeasures();
	}
	/**
	...
	*/
	dataURI(options2) {
		...
		if (this.trellis && this.trellis._mounted) return this.trellis.exports.dataURI(options2);
	...
	*/
	getSvgString(scale) {
		if (!this.ctx.exports) throw new Error("apexcharts: Exports feature is not registered. Import apexcharts/features/exports.");
		if (this.trellis && this.trellis._mounted) return this.trellis.exports.svgString();
	...
	*/
	exportToCSV(options2 = {}) {
		if (!this.ctx.exports) throw new Error("apexcharts: Exports feature is not registered. Import apexcharts/features/exports.");
		if (this.trellis && this.trellis._mounted) return this.trellis.exports.download("csv");
	...
	*/
	_deferParentResize() {
		...
		const speed = ((_a = this.w.config.chart.animations) == null ? void 0 : _a.speed) || 800;
		const giveUpAfter = Math.min(Math.max(1e3 + speed * 2, 1500), 15e3);
		const check = () => {
			this._parentResizeWaiter = null;
			if (this.w.globals.isDestroyed || !Utils$1.elementExists(this.el)) return;
			if (this.w.globals.animationEnded || Date.now() - startedAt >= giveUpAfter) {
				this._windowResize();
				return;
			}
	...
	*/
	_printEnabled() {
		...
		return print !== false && (print == null ? void 0 : print.enabled) !== false;
	}
	/**
	...
	*/
	_beforePrint() {
		...
		const w = this.w;
		const printWidth = (_b = (_a = w.config.chart.print) == null ? void 0 : _a.width) != null ? _b : 700;
		this._printRestore = { width: w.config.chart.width };
		if (typeof printWidth === "number" && printWidth > 0 && w.globals.svgWidth > printWidth) this.updateHelpers._updateOptions({ chart: { width: printWidth } }, false, false, false, false);
		const svg = (_d = (_c = w.dom) == null ? void 0 : _c.Paper) == null ? void 0 : _d.node;
		if (svg && !svg.getAttribute("viewBox")) {
			svg.setAttribute("viewBox", `0 0 ${w.globals.svgWidth} ${w.globals.svgHeight}`);
			this._printRestore.viewBoxAdded = true;
			(_f = (_e = w.dom) == null ? void 0 : _e.elWrap) == null || _f.classList.add("apexcharts-printing");
			const host = this.el;
			if (host && host.style) {
	...
	_afterPrint() {
		...
		if (restore.viewBoxAdded) {
			(_c = (_b = (_a = w.dom) == null ? void 0 : _a.Paper) == null ? void 0 : _b.node) == null || _c.removeAttribute("viewBox");
			(_e = (_d = w.dom) == null ? void 0 : _d.elWrap) == null || _e.classList.remove("apexcharts-printing");
			const host = this.el;
			if (host && host.style) host.style.minHeight = (_f = restore.minHeight) != null ? _f : "";
		}
		if (w.config.chart.width !== restore.width) {
			this.updateHelpers._updateOptions({ chart: { width: restore.width } }, false, false, false, false);
			clearTimeout((_g = w.globals.resizeTimer) != null ? _g : void 0);
		}
	}
	/**
	...
	_windowResize() {
		...
		this.w.globals.resizeTimer = window.setTimeout(() => {
			...
			if (this.core && gl.lastResizeSignature) {
				...
				if (sig.w === gl.lastResizeSignature.w && sig.h === gl.lastResizeSignature.h && sig.iw === gl.lastResizeSignature.iw) return;
			}
			gl.resized = true;
