...
import { $t as rangeContainsRange, B as eventTupleToStore, D as compileEventUis, E as compareNumbers, H as expandRecurring, I as diffDates, J as getElEventRange, L as disableCursor, M as createEmptyEventStore, N as createEventInstance, Ot as whenTransitionDone, Q as getRelevantEvents, R as enableCursor, Tt as triggerDateSelect, W as filterEventStoreDefs, _ as allowSelection, _t as preventContextMenu, a as getAppendableRoot, at as isDateSpansEqual, b as buildDateSpanApiWithContext, bt as refineEventDef, en as rangesIntersect, g as allowContextMenu, ht as parseEventDef, i as computeElIsRtl, kt as classNames, n as applyStyle, o as getEventTargetViaRoot, p as EventImpl, q as getDefaultEventEnd, s as BASE_OPTION_DEFAULTS, t as Emitter, tn as startOfDay, ut as mapHash, v as applyMutationToEventStore, vt as preventSelection, x as buildEventApis, xt as refineProps, zt as createDuration } from "./56f74c4a-SfSX7ugV.js";
import { i as interactionSettingsToStore, n as excludeInstances, r as interactionSettingsStore, t as Interaction } from "./d5a70381-Dij-WEiw.js";
import { a as diffPoints, c as intersectRects, i as constrainPoint, l as pointInsideRect, n as computeInnerRect, o as getClippingParents, r as computeRect, s as getRectCenter } from "./1da0ed53-KO0xoqpu.js";
//#region node_modules/fullcalendar/chunks/e2d9e59e.js
...
function isInteractionPropsValid(state, context, dateSpanMeta, filterConfig) {
	...
	for (let subjectInstanceId in subjectInstances) {
		...
		for (let subjectAllow of subjectConfig.allows) {
			let subjectDateSpan = _objectSpread2(_objectSpread2({}, dateSpanMeta), {}, {
...
function isDateSelectionPropsValid(state, context, dateSpanMeta, filterConfig) {
	...
	for (let selectionAllow of selectionConfig.allows) if (!selectionAllow(buildDateSpanApiWithContext(_objectSpread2(_objectSpread2({}, dateSpanMeta), selection), context), null)) return false;
	return true;
}
...
//#endregion
//#region node_modules/fullcalendar/chunks/68f617f1.js
...
var PointerDragging = class {
	...
	querySubjectEl(ev) {
		if (this.selector) return ev.target.closest(this.selector);
...
var ElementMirror = class {
	...
	setIsVisible(bool) {
		if (bool) {
			if (!this.isVisible) {
				if (this.mirrorEl) this.mirrorEl.style.setProperty("display", "", "important");
				this.isVisible = bool;
				this.updateElPosition();
			}
		} else if (this.isVisible) {
			if (this.mirrorEl) this.mirrorEl.style.setProperty("display", "none", "important");
			this.isVisible = bool;
		}
	}
	...
	getMirrorEl() {
		...
		if (!mirrorEl) {
			...
			if (this.colorScheme) mirrorEl.setAttribute("data-color-scheme", this.colorScheme);
...
var HitDragging = class {
	constructor(dragging, droppableStore) {
		...
		this.handlePointerDown = (ev) => {
			...
			if (this.initialHit || !this.requireInitial) this.emitter.trigger("pointerdown", ev);
			else dragging.cancel();
		};
...
var DateClicking = class extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (pev) => {
			...
			if (!(this.component.context.emitter.hasHandlers("dateClick") && this.component.isValidDateDownEl(downEl))) dragging.cancel();
		};
		this.handleDragEnd = (ev) => {
			let { component } = this;
			...
			if (!pointer.wasTouchScroll) {
				...
				if (initialHit && finalHit && isHitsEqual(initialHit, finalHit)) {
					...
					let data = _objectSpread2(_objectSpread2({}, buildDatePointApiWithContext(initialHit.dateSpan, context)), {}, {
...
var DateSelecting = class extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (ev) => {
			...
			if (!(options.selectable && component.isValidDateDownEl(ev.origEvent.target))) dragging.cancel();
			else dragging.delay = ev.isTouch ? getComponentTouchDelay$1(component) : null;
		};
		this.handleDragStart = (ev) => {
		...
		this.handlePointerUp = (pev) => {
			if (this.dragSelection) {
				...
				this.dragSelection = null;
			} else this.component.context.emitter.trigger("_noDateSelect");
		};
...
var EventDragging = class EventDragging extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (ev) => {
			...
			let eventInstanceId = (this.eventRange = getElEventRange(ev.subjectEl)).instance.instanceId;
			...
			if (!(component.isValidSegDownEl(origTarget) && !origTarget.closest(`.${classNames.internalEventResizer}`))) dragging.cancel();
			else this.isDragging = ev.subjectEl.classList.contains(classNames.internalEventDraggable);
		};
		this.handleDragStart = (ev) => {
			...
			if (this.isDragging) {
				...
				if (validMutation) {
					if (receivingContext === initialContext) {
						...
						initialContext.emitter.trigger("eventDrop", _objectSpread2(_objectSpread2(_objectSpread2({}, eventChangeData), transformed), {}, {
							el: ev.subjectEl,
						...
						initialContext.emitter.trigger("eventLeave", _objectSpread2(_objectSpread2({}, eventRemoveData), {}, {
						...
						receivingContext.emitter.trigger("drop", _objectSpread2(_objectSpread2({}, buildDatePointApiWithContext(finalHit.dateSpan, receivingContext)), {}, {
							draggedEl: ev.subjectEl,
							jsEvent: ev.origEvent,
							view: finalHit.context.viewApi
						}));
						receivingContext.emitter.trigger("eventReceive", _objectSpread2(_objectSpread2({}, eventAddData), {}, {
							draggedEl: ev.subjectEl,
							view: finalHit.context.viewApi
						}));
					}
...
var EventResizing = class extends Interaction {
	constructor(settings) {
		...
		this.handlePointerDown = (ev) => {
			...
			if (!(this.component.isValidSegDownEl(ev.origEvent.target) && !(ev.isTouch && this.component.props.eventSelection !== eventRange.instance.instanceId))) this.dragging.cancel();
		};
		this.handleDragStart = (ev) => {
			let { context } = this.component;
			let eventRange = this.eventRange;
		...
		this.handleHitUpdate = (hit, isFinal, ev) => {
			...
			if (hit) {
				if (!(hit.componentId === initialHit.componentId && this.isHitComboAllowed && !this.isHitComboAllowed(initialHit, hit))) mutation = computeMutation(initialHit, hit, ev.subjectEl.classList.contains(classNames.internalEventResizerStart), eventInstance.range);
			}
			if (mutation) {
		...
		this.handleDragEnd = (ev) => {
			...
			if (this.validMutation) {
				...
				context.emitter.trigger("eventResize", _objectSpread2(_objectSpread2({}, eventChangeData), {}, {
					el: this.draggingSegEl,
...
//#endregion
//#region node_modules/fullcalendar/interaction.js
...
var ExternalElementDragging = class {
	constructor(dragging, suppliedDragMeta) {
		...
		this.handleHitUpdate = (hit, isFinal, ev) => {
			...
			this.displayDrag(receivingContext, interaction);
			dragging.setMirrorIsVisible(isFinal || !droppableEvent || !document.querySelector(`.${classNames.internalEventMirror}`));
			if (!isInvalid) enableCursor();
			else disableCursor();
			if (!isFinal) {
	...
	canDropElOnCalendar(el, receivingContext) {
		...
		if (typeof dropAccept === "string" && dropAccept) return el.matches(dropAccept);
		return true;
	}
};
function computeEventForDateSpan(dateSpan, dragMeta, context) {
	let defProps = _objectSpread2({}, dragMeta.leftoverProps);
...
var ExternalDraggable = class {
	constructor(el, settings = {}) {
		...
		this.handleDragStart = (ev) => {
			if (ev.isTouch && this.dragging.delay && ev.subjectEl.classList.contains(classNames.internalEvent)) this.dragging.mirror.getMirrorEl().classList.add(classNames.internalEventSelected);
		};
...
//#endregion
export { ExternalDraggable as Draggable, ThirdPartyDraggable, interactionPlugin as default };
